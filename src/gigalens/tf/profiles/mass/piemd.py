...
class DPIE(MassProfile):
    ...
    def _complex_deriv_dual(self, x, y, r_core, r_cut, e, q):
        ...
        z_frac_re, z_frac_im = self._optimal_complex_divide_double(
            q * x,
            2. * sqe * tf.math.sqrt(r_core**2 + rem2) - y / q,
            x,
            2. * r_core * sqe - y,
            2. * sqe * tf.math.sqrt(r_cut**2 + rem2) - y / q,
            2. * r_cut * sqe - y
        )
    ...
    def _complex_log(a, b):
        ...
        """
        norm2 = a**2 + b**2
        z_re = tf.math.log(tf.math.sqrt(norm2))
        z_im = tf.math.atan2(b, a)
        return z_re, z_im
