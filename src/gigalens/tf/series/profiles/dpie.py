import tensorflow
from tensorflow.math import atan2
from tensorflow.math import log
from tensorflow.math import sqrt
from tensorflow.math import pow


@tensorflow.function
def deriv_0(x, y, e, r_core, r_cut):
    x0 = -y
    x1 = sqrt(e)
    x2 = 2.0*x1
    x3 = r_core*x2 + x0
    x4 = e + 1
    x5 = 1 - e
    x6 = x5/x4
    x7 = x*x6
    x8 = -x4*y/x5
    x9 = pow(x, 2)
    x10 = x9/pow(e + 1.0, 2) + pow(y, 2)/pow(1.0 - e, 2)
    x11 = x2*sqrt(pow(r_cut, 2) + x10) + x8
    x12 = x*x11 + x3*x7
    x13 = x6*x9
    x14 = -x11*x3 + x13
    x15 = pow(x12, 2) + pow(x14, 2)
    x16 = pow(x15, -1)
    x17 = r_cut*x2 + x0
    x18 = x2*sqrt(pow(r_core, 2) + x10) + x8
    x19 = x*x18 + x17*x7
    x20 = x13 - x17*x18
    x21 = -x12*x20 + x14*x19
    x22 = x12*x19 + x14*x20
    x23 = r_cut*(0.5*pow(e, 2) - 0.5)/(x1*(-r_core + r_cut))
    x24 = pow(x15, -2)
    return [-x23*atan2(x16*x21, x16*x22), x23*log(sqrt(pow(x21, 2)*x24 + pow(x22, 2)*x24))]


@tensorflow.function
def deriv_1(x, y, e, r_core, r_cut):
    x0 = -y
    x1 = sqrt(e)
    x2 = 2.0*x1
    x3 = r_core*x2 + x0
    x4 = e + 1
    x5 = pow(x4, -1)
    x6 = 1 - e
    x7 = x5*x6
    x8 = x*x7
    x9 = -x4*y/x6
    x10 = pow(x, 2)
    x11 = x10/pow(e + 1.0, 2) + pow(y, 2)/pow(1.0 - e, 2)
    x12 = sqrt(pow(r_cut, 2) + x11)
    x13 = x12*x2 + x9
    x14 = x*x13 + x3*x8
    x15 = x10*x7
    x16 = -x13*x3 + x15
    x17 = pow(x14, 2) + pow(x16, 2)
    x18 = pow(x17, -1)
    x19 = r_cut*x2
    x20 = x0 + x19
    x21 = x2*sqrt(pow(r_core, 2) + x11) + x9
    x22 = x*x21 + x20*x8
    x23 = x15 - x20*x21
    x24 = -x14*x23 + x16*x22
    x25 = x18*x24
    x26 = x14*x22 + x16*x23
    x27 = x18*x26
    x28 = atan2(x25, x27)
    x29 = -r_core + r_cut
    x30 = pow(x1, -1)
    x31 = 0.5*pow(e, 2) - 0.5
    x32 = x30*x31
    x33 = x32/x29
    x34 = pow(x29, -2)
    x35 = pow(x17, -2)
    x36 = pow(x26, 2)
    x37 = pow(x24, 2)
    x38 = x35*x36 + x35*x37
    x39 = pow(x38, -1)
    x40 = pow(x12, -1)
    x41 = x19*x40
    x42 = x*x41
    x43 = x3*x41
    x44 = 4.0*x1
    x45 = x14*x44
    x46 = r_cut*x40
    x47 = x*x46
    x48 = x35*(4.0*r_cut*x1*x16*x3*x40 - x45*x47)
    x49 = r_cut*x33
    x50 = log(sqrt(x38))
    x51 = (1/2)*(8.0*r_cut*x1*x16*x3*x40 - 8.0*x1*x14*x47)/pow(x17, 3)
    x52 = x44*x47
    x53 = x3*x44*x46
    x54 = (1/2)*x35
    return [r_cut*x28*x30*x31*x34 - x28*x33 - x49*(-x25*x39*(x18*(x14*x2*x8 - x16*x2*x21 + x22*x42 - x23*x43) + x26*x48) + x27*x39*(x18*(2.0*x*x1*x16*x5*x6 + 2.0*x1*x14*x21 - x22*x43 - x23*x42) + x24*x48)), -r_cut*x32*x34*x50 + x33*x50 + x39*x49*(x24*x54*(4.0*x*x1*x16*x5*x6 + 4.0*x1*x14*x21 - x22*x53 - x23*x52) + x26*x54*(-x16*x21*x44 + x22*x52 - x23*x53 + x45*x8) + x36*x51 + x37*x51)]


@tensorflow.function
def deriv_2(x, y, e, r_core, r_cut):
    x0 = sqrt(e)
    x1 = pow(x0, -1)
    x2 = 0.5*pow(e, 2) - 0.5
    x3 = -r_core + r_cut
    x4 = pow(x3, -2)
    x5 = -y
    x6 = r_core*x0
    x7 = x5 + 2.0*x6
    x8 = e + 1
    x9 = pow(x8, -1)
    x10 = 1 - e
    x11 = x10*x9
    x12 = x*x11
    x13 = x12*x7
    x14 = -x8*y/x10
    x15 = pow(r_cut, 2)
    x16 = pow(x, 2)
    x17 = x16/pow(e + 1.0, 2) + pow(y, 2)/pow(1.0 - e, 2)
    x18 = x15 + x17
    x19 = sqrt(x18)
    x20 = 2.0*x0
    x21 = x14 + x19*x20
    x22 = x*x21 + x13
    x23 = x11*x16
    x24 = -x21*x7 + x23
    x25 = pow(x22, 2) + pow(x24, 2)
    x26 = pow(x25, -1)
    x27 = r_cut*x20
    x28 = x27 + x5
    x29 = x14 + x20*sqrt(pow(r_core, 2) + x17)
    x30 = x*x29
    x31 = x12*x28 + x30
    x32 = x23 - x28*x29
    x33 = -x22*x32 + x24*x31
    x34 = x26*x33
    x35 = x22*x31 + x24*x32
    x36 = x26*x35
    x37 = atan2(x34, x36)
    x38 = 2*x1*x2
    x39 = r_cut*x38/pow(x3, 3)
    x40 = pow(x3, -1)
    x41 = pow(x25, -2)
    x42 = pow(x35, 2)
    x43 = pow(x33, 2)
    x44 = x41*x42 + x41*x43
    x45 = pow(x44, -1)
    x46 = x*x32
    x47 = pow(x19, -1)
    x48 = x27*x47
    x49 = x31*x48
    x50 = 2.0*x*x0*x10*x24*x9 + 2.0*x0*x22*x29 - x46*x48 - x49*x7
    x51 = x26*x50
    x52 = 4.0*x0
    x53 = x47*x52
    x54 = x*x22
    x55 = x53*x54
    x56 = 4.0*r_cut*x0*x24*x47*x7 - r_cut*x55
    x57 = x41*x56
    x58 = x33*x57
    x59 = x51 + x58
    x60 = x45*x59
    x61 = x32*x7
    x62 = x*x49 + x12*x20*x22 - x20*x24*x29 - x48*x61
    x63 = x26*x62
    x64 = x35*x57
    x65 = x63 + x64
    x66 = x45*x65
    x67 = -x34*x66 + x36*x60
    x68 = 8.0*e
    x69 = r_cut*x47
    x70 = x68*x69
    x71 = x20*x47
    x72 = pow(x18, -3/2)
    x73 = x15*x72
    x74 = x20*x73
    x75 = x31*x71
    x76 = x31*x74
    x77 = 2*x57
    x78 = pow(x25, -3)
    x79 = 8.0*x0
    x80 = x47*x54*x79
    x81 = 8.0*r_cut*x0*x24*x47*x7 - r_cut*x80
    x82 = x78*x81
    x83 = x56*x82
    x84 = pow(x18, -1)
    x85 = x15*x16*x84
    x86 = 4.0*pow(x6 - 0.5*y, 2)
    x87 = x24*x7
    x88 = x52*x73
    x89 = x41*(-8.0*e*x15*x84*x86 + 4.0*x*x0*x15*x22*x72 + 4.0*x0*x24*x47*x7 - x55 - x68*x85 - x87*x88)
    x90 = x29*x7
    x91 = x42*x82
    x92 = x43*x82
    x93 = x31*x53
    x94 = x*x93
    x95 = x53*x61
    x96 = r_cut*x94 - r_cut*x95 + x12*x22*x52 - x24*x29*x52
    x97 = x35*x41
    x98 = x96*x97
    x99 = x46*x53
    x100 = x7*x93
    x101 = -r_cut*x100 - r_cut*x99 + 4.0*x*x0*x10*x24*x9 + 4.0*x0*x22*x29
    x102 = x33*x41
    x103 = x101*x102
    x104 = (-x103 - x91 - x92 - x98)/pow(x44, 2)
    x105 = r_cut*x1*x2*x40
    x106 = log(sqrt(x44))
    x107 = x38*x4
    x108 = (1/2)*x103 + (1/2)*x91 + (1/2)*x92 + (1/2)*x98
    x109 = x108*x45
    x110 = (1/2)*x42
    x111 = x81*(12.0*r_cut*x0*x24*x47*x7 - 12.0*x0*x54*x69)/pow(x25, 4)
    x112 = (1/2)*x43
    x113 = 16.0*e
    x114 = x78*(-16.0*e*x15*x84*x86 + 8.0*x*x0*x15*x22*x72 + 8.0*x0*x24*x47*x7 - x113*x85 - x73*x79*x87 - x80)
    x115 = x113*x69
    x116 = x31*x88
    x117 = (1/2)*x41
    return [2*r_cut*x1*x2*x4*x67 + 2*x1*x2*x37*x4 - x105*(-x104*x34*x65 + x104*x36*x59 - x34*x45*(x26*(x*x75 - x*x76 + x23*x70 - x61*x71 + x61*x74 + x70*x90) + x35*x83 + x35*x89 + x62*x77) + x36*x45*(x26*(-x13*x70 + x30*x70 - x46*x71 + x46*x74 - x7*x75 + x7*x76) + x33*x83 + x33*x89 + x50*x77) - x51*x66 - x58*x66 + x60*x63 + x60*x64) - x37*x39 - x38*x40*x67, -r_cut*x107*x109 + x104*x105*x108 + x105*x45*(x101*x117*x50 + x101*x33*x82 + (1/2)*x102*(-x100 - x115*x13 + x115*x30 + x116*x7 + x46*x88 - x99) + x110*x111 + x110*x114 + x111*x112 + x112*x114 + x117*x62*x96 + x35*x82*x96 + (1/2)*x97*(-x*x116 + x115*x23 + x115*x90 + x61*x88 + x94 - x95)) - x106*x107 + x106*x39 + x109*x38*x40]


@tensorflow.function
def deriv_3(x, y, e, r_core, r_cut):
    x0 = -y
    x1 = sqrt(e)
    x2 = r_core*x1
    x3 = x0 + 2.0*x2
    x4 = e + 1
    x5 = pow(x4, -1)
    x6 = 1 - e
    x7 = x5*x6
    x8 = x*x7
    x9 = x3*x8
    x10 = -x4*y/x6
    x11 = pow(r_cut, 2)
    x12 = pow(x, 2)
    x13 = x12/pow(e + 1.0, 2) + pow(y, 2)/pow(1.0 - e, 2)
    x14 = x11 + x13
    x15 = sqrt(x14)
    x16 = 2.0*x1
    x17 = x10 + x15*x16
    x18 = x*x17 + x9
    x19 = x12*x7
    x20 = -x17*x3 + x19
    x21 = pow(x18, 2) + pow(x20, 2)
    x22 = pow(x21, -1)
    x23 = r_cut*x16
    x24 = x0 + x23
    x25 = x10 + x16*sqrt(pow(r_core, 2) + x13)
    x26 = x*x25
    x27 = x24*x8 + x26
    x28 = x19 - x24*x25
    x29 = -x18*x28 + x20*x27
    x30 = x22*x29
    x31 = x18*x27 + x20*x28
    x32 = x22*x31
    x33 = atan2(x30, x32)
    x34 = -r_core + r_cut
    x35 = pow(x1, -1)
    x36 = 0.5*pow(e, 2) - 0.5
    x37 = x35*x36
    x38 = 6*x37
    x39 = x38/pow(x34, 3)
    x40 = pow(x34, -4)
    x41 = pow(x34, -2)
    x42 = pow(x21, -2)
    x43 = pow(x31, 2)
    x44 = pow(x29, 2)
    x45 = x42*x43 + x42*x44
    x46 = pow(x45, -1)
    x47 = x*x28
    x48 = pow(x15, -1)
    x49 = x23*x48
    x50 = x27*x49
    x51 = 2.0*x*x1*x20*x5*x6 + 2.0*x1*x18*x25 - x3*x50 - x47*x49
    x52 = x22*x51
    x53 = 4.0*x1
    x54 = x48*x53
    x55 = x*x18
    x56 = x54*x55
    x57 = 4.0*r_cut*x1*x20*x3*x48 - r_cut*x56
    x58 = x29*x42
    x59 = x57*x58
    x60 = x52 + x59
    x61 = x46*x60
    x62 = x28*x3
    x63 = x*x50 + x16*x18*x8 - x16*x20*x25 - x49*x62
    x64 = x22*x63
    x65 = x31*x42
    x66 = x57*x65
    x67 = x64 + x66
    x68 = x46*x67
    x69 = -x30*x68 + x32*x61
    x70 = r_cut*x39
    x71 = x37/x34
    x72 = e*x48
    x73 = r_cut*x72
    x74 = 8.0*x73
    x75 = x16*x48
    x76 = pow(x14, -3/2)
    x77 = x11*x76
    x78 = x16*x77
    x79 = x27*x75
    x80 = x27*x78
    x81 = x26*x74 - x3*x79 + x3*x80 - x47*x75 + x47*x78 - x74*x9
    x82 = x22*x81
    x83 = x42*x51
    x84 = 2*x57
    x85 = x83*x84
    x86 = pow(x21, -3)
    x87 = x1*x48
    x88 = 8.0*x55*x87
    x89 = 8.0*r_cut*x1*x20*x3*x48 - r_cut*x88
    x90 = x86*x89
    x91 = x57*x90
    x92 = x29*x91
    x93 = 8.0*x11
    x94 = pow(x14, -1)
    x95 = e*x12*x94
    x96 = pow(x2 - 0.5*y, 2)
    x97 = 4.0*x96
    x98 = x20*x3
    x99 = x53*x77
    x100 = -8.0*e*x11*x94*x97 + 4.0*x*x1*x11*x18*x76 + 4.0*x1*x20*x3*x48 - x56 - x93*x95 - x98*x99
    x101 = x100*x58
    x102 = x101 + x82 + x85 + x92
    x103 = x32*x46
    x104 = x25*x3
    x105 = x*x79 - x*x80 + x104*x74 + x19*x74 - x62*x75 + x62*x78
    x106 = x105*x22
    x107 = x42*x63
    x108 = x107*x84
    x109 = x31*x91
    x110 = x100*x65
    x111 = x106 + x108 + x109 + x110
    x112 = x30*x46
    x113 = pow(x45, -2)
    x114 = x43*x90
    x115 = x44*x90
    x116 = x27*x54
    x117 = x*x116
    x118 = x54*x62
    x119 = r_cut*x117 - r_cut*x118 + x18*x53*x8 - x20*x25*x53
    x120 = x119*x65
    x121 = x47*x54
    x122 = x116*x3
    x123 = -r_cut*x121 - r_cut*x122 + 4.0*x*x1*x20*x5*x6 + 4.0*x1*x18*x25
    x124 = x123*x58
    x125 = -x114 - x115 - x120 - x124
    x126 = x113*x125
    x127 = x32*x60
    x128 = x30*x67
    x129 = x102*x103 - x111*x112 + x126*x127 - x126*x128 - x52*x68 - x59*x68 + x61*x64 + x61*x66
    x130 = 2*x46
    x131 = x111*x130
    x132 = x102*x130
    x133 = 2*x126
    x134 = x133*x67
    x135 = x133*x60
    x136 = x125*(-2*x114 - 2*x115 - 2*x120 - 2*x124)/pow(x45, 3)
    x137 = 12.0*x72
    x138 = e*x77
    x139 = 12.0*x138
    x140 = 6.0*x1
    x141 = pow(r_cut, 3)
    x142 = pow(x14, -5/2)
    x143 = x141*x142
    x144 = x27*x3
    x145 = x140*x143
    x146 = x42*x81
    x147 = 3*x57
    x148 = 3*x91
    x149 = 3*x100
    x150 = pow(x21, -4)
    x151 = 12.0*x55*x87
    x152 = 12.0*r_cut*x1*x20*x3*x48 - r_cut*x151
    x153 = x150*x152
    x154 = x153*x89
    x155 = x154*x57
    x156 = r_cut*x95
    x157 = pow(x14, -2)
    x158 = e*x96
    x159 = 12.0*x1
    x160 = x143*x159
    x161 = r_cut*x76
    x162 = x159*x161
    x163 = -8.0*e*r_cut*x94*x97 + 24.0*e*x12*x141*x157 + 64.0*e*x141*x157*x96 + 8.0*e*x141*x157*x97 + 12.0*r_cut*x*x1*x18*x76 - 64.0*r_cut*x158*x94 + 12.0*x1*x141*x142*x20*x3 - 24.0*x156 - x160*x55 - x162*x98
    x164 = x11*x95
    x165 = x1*x98
    x166 = -16.0*e*x11*x94*x97 + 8.0*x*x1*x11*x18*x76 + 8.0*x1*x20*x3*x48 - 16.0*x164 - x165*x76*x93 - x88
    x167 = x166*x86
    x168 = x167*x57
    x169 = x29*x90
    x170 = 2*x100
    x171 = x140*x161
    x172 = x*x27
    x173 = x105*x42
    x174 = x31*x90
    x175 = x154*x43
    x176 = x154*x44
    x177 = x167*x43
    x178 = x167*x44
    x179 = 16.0*x73
    x180 = x104*x179 + x117 - x118 - x172*x99 + x179*x19 + x62*x99
    x181 = x180*x65
    x182 = -x121 - x122 + x144*x99 + x179*x26 - x179*x9 + x47*x99
    x183 = x182*x58
    x184 = x123*x83
    x185 = x107*x119
    x186 = x119*x174
    x187 = x123*x169
    x188 = x113*(-x175 - x176 - x177 - x178 - x181 - x183 - x184 - x185 - 2*x186 - 2*x187)
    x189 = r_cut*x71
    x190 = log(sqrt(x45))
    x191 = (1/2)*x114 + (1/2)*x115 + (1/2)*x120 + (1/2)*x124
    x192 = x191*x46
    x193 = 3*x71
    x194 = x126*x191
    x195 = 3*r_cut*x37*x41
    x196 = x189*x191
    x197 = (1/2)*x175 + (1/2)*x176 + (1/2)*x177 + (1/2)*x178 + (1/2)*x181 + (1/2)*x183 + (1/2)*x184 + (1/2)*x185 + x186 + x187
    x198 = x197*x46
    x199 = (1/2)*x43
    x200 = x199*x89
    x201 = x152*(16.0*r_cut*x1*x20*x3*x48 - 16.0*r_cut*x55*x87)/pow(x21, 5)
    x202 = (1/2)*x44
    x203 = x202*x89
    x204 = x86*(-16.0*e*r_cut*x94*x97 + 48.0*e*x12*x141*x157 + 128.0*e*x141*x157*x96 + 16.0*e*x141*x157*x97 + 24.0*r_cut*x*x1*x18*x76 - 128.0*r_cut*x158*x94 + 24.0*x1*x141*x142*x20*x3 - 24.0*x1*x143*x55 - 48.0*x156 - 24.0*x161*x165)
    x205 = 24.0*x72
    x206 = 24.0*x138
    x207 = x153*x166
    x208 = x150*(-24.0*e*x11*x94*x97 + 12.0*x*x1*x11*x18*x76 + 12.0*x1*x20*x3*x48 - x151 - x159*x77*x98 - 24.0*x164)
    x209 = (3/2)*x90
    x210 = x119*x31
    x211 = (3/2)*x154
    x212 = x123*x29
    x213 = (3/2)*x167
    return [3*r_cut*x129*x35*x36*x41 + 6*r_cut*x33*x35*x36*x40 - 3*x129*x71 - x189*(-x101*x68 + x102*x133*x32 + x103*(x146*x147 + x148*x51 + x149*x83 + x155*x29 + x163*x58 + x168*x29 + x169*x170 + x22*(12.0*e*x*x11*x3*x5*x6*x76 + 12.0*e*x*x25*x48 + 6.0*r_cut*x*x1*x28*x76 + 6.0*r_cut*x1*x27*x3*x76 - x137*x9 - x139*x26 - x140*x143*x47 - x144*x145)) + x106*x61 + x108*x61 + x109*x61 + x110*x61 - x111*x133*x30 - x112*(x107*x149 + x147*x173 + x148*x63 + x155*x31 + x163*x65 + x168*x31 + x170*x174 + x22*(x104*x137 - x104*x139 + x137*x19 - x139*x19 + x145*x172 - x145*x62 - x171*x172 + x171*x62)) + x127*x136 + x127*x188 - x128*x136 - x128*x188 - x131*x52 - x131*x59 + x132*x64 + x132*x66 - x134*x52 - x134*x59 + x135*x64 + x135*x66 - x68*x82 - x68*x85 - x68*x92) - x33*x39 + 6*x35*x36*x41*x69 - x69*x70, -r_cut*x190*x38*x40 + x133*x189*x197 + x136*x196 + x188*x196 + x189*x46*(x107*x180 + (1/2)*x119*x173 + x119*x209*x63 + (1/2)*x123*x146 + x123*x209*x51 + (3/2)*x169*x182 + (3/2)*x174*x180 + x182*x83 + x199*x204 + x200*x201 + x200*x208 + x201*x203 + x202*x204 + x203*x208 + x207*x43 + x207*x44 + x210*x211 + x210*x213 + x211*x212 + x212*x213 + (1/2)*x58*(24.0*e*x*x11*x3*x5*x6*x76 + 24.0*e*x*x25*x48 + 12.0*r_cut*x*x1*x28*x76 + 12.0*r_cut*x1*x27*x3*x76 - x144*x160 - x160*x47 - x205*x9 - x206*x26) + (1/2)*x65*(x104*x205 - x104*x206 + x160*x172 - x160*x62 - x162*x172 + x162*x62 + x19*x205 - x19*x206)) + x190*x39 - x192*x38*x41 + x192*x70 + x193*x194 + x193*x198 - x194*x195 - x195*x198]


@tensorflow.function
def deriv_4(x, y, e, r_core, r_cut):
    x0 = sqrt(e)
    x1 = pow(x0, -1)
    x2 = 0.5*pow(e, 2) - 0.5
    x3 = -r_core + r_cut
    x4 = pow(x3, -4)
    x5 = -y
    x6 = r_core*x0
    x7 = x5 + 2.0*x6
    x8 = e + 1
    x9 = pow(x8, -1)
    x10 = 1 - e
    x11 = x10*x9
    x12 = x*x11
    x13 = x12*x7
    x14 = -x8*y/x10
    x15 = pow(r_cut, 2)
    x16 = pow(x, 2)
    x17 = x16/pow(e + 1.0, 2) + pow(y, 2)/pow(1.0 - e, 2)
    x18 = x15 + x17
    x19 = sqrt(x18)
    x20 = 2.0*x0
    x21 = x14 + x19*x20
    x22 = x*x21 + x13
    x23 = x11*x16
    x24 = -x21*x7 + x23
    x25 = pow(x22, 2) + pow(x24, 2)
    x26 = pow(x25, -1)
    x27 = r_cut*x20
    x28 = x27 + x5
    x29 = x14 + x20*sqrt(pow(r_core, 2) + x17)
    x30 = x*x29
    x31 = x12*x28 + x30
    x32 = x23 - x28*x29
    x33 = -x22*x32 + x24*x31
    x34 = x26*x33
    x35 = x22*x31 + x24*x32
    x36 = x26*x35
    x37 = atan2(x34, x36)
    x38 = x1*x2
    x39 = 24*x38
    x40 = r_cut*x39/pow(x3, 5)
    x41 = pow(x3, -3)
    x42 = pow(x25, -2)
    x43 = pow(x35, 2)
    x44 = pow(x33, 2)
    x45 = x42*x43 + x42*x44
    x46 = pow(x45, -1)
    x47 = x*x32
    x48 = pow(x19, -1)
    x49 = x27*x48
    x50 = x31*x7
    x51 = 2.0*x*x0*x10*x24*x9 + 2.0*x0*x22*x29 - x47*x49 - x49*x50
    x52 = x26*x51
    x53 = 4.0*x0
    x54 = x48*x53
    x55 = x*x22
    x56 = x54*x55
    x57 = 4.0*r_cut*x0*x24*x48*x7 - r_cut*x56
    x58 = x42*x57
    x59 = x33*x58
    x60 = x52 + x59
    x61 = x46*x60
    x62 = x*x31
    x63 = x32*x7
    x64 = x12*x20*x22 - x20*x24*x29 + x49*x62 - x49*x63
    x65 = x26*x64
    x66 = x35*x58
    x67 = x65 + x66
    x68 = x46*x67
    x69 = -x34*x68 + x36*x61
    x70 = pow(x3, -2)
    x71 = 8.0*x48
    x72 = r_cut*x71
    x73 = e*x13
    x74 = e*x30
    x75 = x20*x48
    x76 = x32*x75
    x77 = pow(x18, -3/2)
    x78 = x15*x77
    x79 = x20*x78
    x80 = -x*x76 + x47*x79 - x50*x75 + x50*x79 - x72*x73 + x72*x74
    x81 = x26*x80
    x82 = 2*x58
    x83 = x51*x82
    x84 = pow(x25, -3)
    x85 = x0*x55
    x86 = x71*x85
    x87 = 8.0*r_cut*x0*x24*x48*x7 - r_cut*x86
    x88 = x84*x87
    x89 = x57*x88
    x90 = x33*x89
    x91 = pow(x18, -1)
    x92 = e*x91
    x93 = x16*x92
    x94 = x15*x93
    x95 = pow(x6 - 0.5*y, 2)
    x96 = 4.0*x95
    x97 = x24*x7
    x98 = x53*x78
    x99 = -8.0*e*x15*x91*x96 + 4.0*x*x0*x15*x22*x77 + 4.0*x0*x24*x48*x7 - x56 - 8.0*x94 - x97*x98
    x100 = x42*x99
    x101 = x100*x33
    x102 = x101 + x81 + x83 + x90
    x103 = x36*x46
    x104 = e*x23
    x105 = x29*x7
    x106 = e*x105
    x107 = x104*x72 + x106*x72 + x62*x75 - x62*x79 + x63*x79 - x7*x76
    x108 = x107*x26
    x109 = x64*x82
    x110 = x35*x89
    x111 = x100*x35
    x112 = x108 + x109 + x110 + x111
    x113 = x34*x46
    x114 = x36*x60
    x115 = pow(x45, -2)
    x116 = x43*x88
    x117 = x44*x88
    x118 = x54*x62
    x119 = x32*x54
    x120 = x119*x7
    x121 = r_cut*x118 - r_cut*x120 + x12*x22*x53 - x24*x29*x53
    x122 = x35*x42
    x123 = x121*x122
    x124 = x*x119
    x125 = x50*x54
    x126 = -r_cut*x124 - r_cut*x125 + 4.0*x*x0*x10*x24*x9 + 4.0*x0*x22*x29
    x127 = x33*x42
    x128 = x126*x127
    x129 = -x116 - x117 - x123 - x128
    x130 = x115*x129
    x131 = x34*x67
    x132 = x102*x103 - x112*x113 + x114*x130 - x130*x131 - x52*x68 - x59*x68 + x61*x65 + x61*x66
    x133 = r_cut*x38*x41
    x134 = x38/x3
    x135 = 2*x46
    x136 = x112*x52
    x137 = x102*x135
    x138 = x112*x59
    x139 = 2*x130
    x140 = x52*x67
    x141 = x60*x65
    x142 = x60*x66
    x143 = x59*x67
    x144 = x102*x36
    x145 = x112*x34
    x146 = -2*x116 - 2*x117 - 2*x123 - 2*x128
    x147 = pow(x45, -3)
    x148 = x129*x147
    x149 = x146*x148
    x150 = 12.0*x48
    x151 = e*x150
    x152 = 12.0*x77
    x153 = x15*x152
    x154 = x0*x47
    x155 = pow(r_cut, 3)
    x156 = pow(x18, -5/2)
    x157 = x155*x156
    x158 = 6.0*x157
    x159 = x0*x158
    x160 = 12.0*e*x*x10*x15*x7*x77*x9 + 12.0*e*x*x29*x48 + 6.0*r_cut*x*x0*x32*x77 + 6.0*r_cut*x0*x31*x7*x77 - x13*x151 - x153*x74 - x154*x158 - x159*x50
    x161 = x160*x26
    x162 = 3*x58
    x163 = x162*x80
    x164 = 3*x89
    x165 = x164*x51
    x166 = 3*x100
    x167 = x166*x51
    x168 = x33*x57
    x169 = x150*x85
    x170 = 12.0*r_cut*x0*x24*x48*x7 - r_cut*x169
    x171 = pow(x25, -4)
    x172 = x171*x87
    x173 = x170*x172
    x174 = x168*x173
    x175 = 24.0*x93
    x176 = pow(x18, -2)
    x177 = x92*x95
    x178 = r_cut*x177
    x179 = 12.0*x157
    x180 = x0*x97
    x181 = x152*x180
    x182 = -8.0*e*r_cut*x91*x96 + 24.0*e*x155*x16*x176 + 64.0*e*x155*x176*x95 + 8.0*e*x155*x176*x96 + 12.0*r_cut*x*x0*x22*x77 - r_cut*x175 - r_cut*x181 + 12.0*x0*x155*x156*x24*x7 - 64.0*x178 - x179*x85
    x183 = x182*x42
    x184 = x183*x33
    x185 = -16.0*e*x15*x91*x96 + 8.0*x*x0*x15*x22*x77 + 8.0*x0*x24*x48*x7 - 8.0*x180*x78 - x86 - 16.0*x94
    x186 = x185*x84
    x187 = x186*x57
    x188 = x187*x33
    x189 = x33*x88
    x190 = 2*x99
    x191 = x189*x190
    x192 = x161 + x163 + x165 + x167 + x174 + x184 + x188 + x191
    x193 = x0*x77
    x194 = 6.0*x193
    x195 = x194*x62
    x196 = r_cut*x194*x63 - r_cut*x195 - x104*x153 + x105*x151 - x106*x153 + x151*x23 + x159*x62 - x159*x63
    x197 = x196*x26
    x198 = x107*x162
    x199 = x164*x64
    x200 = x35*x57
    x201 = x173*x200
    x202 = x166*x64
    x203 = x183*x35
    x204 = x187*x35
    x205 = x35*x88
    x206 = x190*x205
    x207 = x197 + x198 + x199 + x201 + x202 + x203 + x204 + x206
    x208 = x170*x43
    x209 = x172*x208
    x210 = x172*x44
    x211 = x170*x210
    x212 = x186*x43
    x213 = x186*x44
    x214 = 16.0*x48
    x215 = r_cut*x214
    x216 = x104*x215 + x106*x215 + x118 - x120 - x62*x98 + x63*x98
    x217 = x122*x216
    x218 = -x124 - x125 - x215*x73 + x215*x74 + x47*x98 + x50*x98
    x219 = x127*x218
    x220 = x126*x42
    x221 = x220*x51
    x222 = x121*x42
    x223 = x222*x64
    x224 = x121*x205
    x225 = x126*x189
    x226 = -x209 - x211 - x212 - x213 - x217 - x219 - x221 - x223 - 2*x224 - 2*x225
    x227 = x115*x226
    x228 = -x101*x68 + x103*x192 + x108*x61 + x109*x61 + x110*x61 + x111*x61 - x113*x207 + x114*x149 + x114*x227 - x131*x149 - x131*x227 - x135*x136 - x135*x138 + x137*x65 + x137*x66 - x139*x140 + x139*x141 + x139*x142 - x139*x143 + x139*x144 - x139*x145 - x68*x81 - x68*x83 - x68*x90
    x229 = 3*x46
    x230 = x112*x229
    x231 = x102*x229
    x232 = x51*x58
    x233 = 6*x46
    x234 = x58*x64
    x235 = 3*x130
    x236 = x235*x67
    x237 = x235*x60
    x238 = 6*x130
    x239 = x102*x238
    x240 = 3*x149
    x241 = x207*x229
    x242 = x192*x229
    x243 = x114*x146
    x244 = x129*(-3*x116 - 3*x117 - 3*x123 - 3*x128)/pow(x45, 4)
    x245 = x131*x146
    x246 = 3*x227
    x247 = x148*(-2*x209 - 2*x211 - 2*x212 - 2*x213 - 2*x217 - 2*x219 - 2*x221 - 2*x223 - 4*x224 - 4*x225)
    x248 = 2*x147*x226
    x249 = 4*x58
    x250 = r_cut*x77
    x251 = 48.0*x73
    x252 = 48.0*x74
    x253 = x15*x156
    x254 = 36.0*x253
    x255 = pow(r_cut, 4)
    x256 = pow(x18, -7/2)
    x257 = x255*x256
    x258 = 30.0*x257
    x259 = x0*x50
    x260 = 6*x89
    x261 = 6*x100
    x262 = 4*x173*x57
    x263 = 4*x183
    x264 = x51*x88
    x265 = 8*x99
    x266 = 4*x187
    x267 = x214*x85
    x268 = 16.0*r_cut*x0*x24*x48*x7 - r_cut*x267
    x269 = pow(x25, -5)
    x270 = x269*x87
    x271 = x268*x270
    x272 = x168*x170
    x273 = 48.0*x93
    x274 = 24.0*x77
    x275 = x180*x274
    x276 = -16.0*e*r_cut*x91*x96 + 48.0*e*x155*x16*x176 + 128.0*e*x155*x176*x95 + 16.0*e*x155*x176*x96 + 24.0*r_cut*x*x0*x22*x77 - r_cut*x273 - r_cut*x275 + 24.0*x0*x155*x156*x24*x7 - 24.0*x157*x85 - 128.0*x178
    x277 = x276*x84
    x278 = x277*x57
    x279 = 3*x182
    x280 = -24.0*e*x15*x91*x96 + 12.0*x*x0*x15*x22*x77 + 12.0*x0*x24*x48*x7 - x15*x175 - x15*x181 - x169
    x281 = x172*x280
    x282 = 3*x173
    x283 = x282*x99
    x284 = x171*x185
    x285 = 2*x284
    x286 = pow(x18, -3)
    x287 = e*x255*x286
    x288 = x16*x287
    x289 = x287*x95
    x290 = 72.0*x253
    x291 = 60.0*x257
    x292 = 144.0*e*x15*x16*x176 + 480.0*e*x15*x176*x95 + 24.0*e*x15*x176*x96 - 24.0*e*x255*x286*x96 + 60.0*x*x0*x22*x255*x256 + 12.0*x*x0*x22*x77 + 72.0*x0*x15*x156*x24*x7 - x175 - 96.0*x177 - x180*x291 - x181 - 120.0*x288 - 384.0*x289 - x290*x85
    x293 = 3*x186
    x294 = x293*x99
    x295 = x170*x200
    x296 = x64*x88
    x297 = x208*x271
    x298 = x170*x44
    x299 = x271*x298
    x300 = x277*x43
    x301 = x277*x44
    x302 = 24.0*x48
    x303 = x15*x274
    x304 = 24.0*e*x*x10*x15*x7*x77*x9 + 24.0*e*x*x29*x48 + 12.0*r_cut*x*x0*x32*x77 + 12.0*r_cut*x0*x31*x7*x77 - x154*x179 - x179*x259 - x302*x73 - x303*x74
    x305 = x127*x304
    x306 = 12.0*x193
    x307 = x306*x62
    x308 = x0*x179
    x309 = r_cut*x306*x63 - r_cut*x307 + x104*x302 - x104*x303 + x106*x302 - x106*x303 + x308*x62 - x308*x63
    x310 = x122*x309
    x311 = x220*x80
    x312 = x42*x51
    x313 = x218*x312
    x314 = x107*x222
    x315 = x42*x64
    x316 = x216*x315
    x317 = x281*x43
    x318 = x208*x284
    x319 = x210*x280
    x320 = x284*x298
    x321 = x205*x216
    x322 = x189*x218
    x323 = x126*x264
    x324 = x121*x296
    x325 = x121*x35
    x326 = x126*x33
    x327 = x115*(-x282*x325 - x282*x326 - x293*x325 - x293*x326 - x297 - x299 - x300 - x301 - x305 - x310 - x311 - 2*x313 - x314 - 2*x316 - x317 - 2*x318 - x319 - 2*x320 - 3*x321 - 3*x322 - 3*x323 - 3*x324)
    x328 = r_cut*x134
    x329 = log(sqrt(x45))
    x330 = x39*x4
    x331 = (1/2)*x116 + (1/2)*x117 + (1/2)*x123 + (1/2)*x128
    x332 = x331*x46
    x333 = x38*x70
    x334 = 12*x333
    x335 = x130*x331
    x336 = 12*x133
    x337 = 4*x134
    x338 = x149*x331
    x339 = r_cut*x333
    x340 = 4*x339
    x341 = (1/2)*x209 + (1/2)*x211 + (1/2)*x212 + (1/2)*x213 + (1/2)*x217 + (1/2)*x219 + (1/2)*x221 + (1/2)*x223 + x224 + x225
    x342 = x341*x46
    x343 = x328*x331
    x344 = x146*x343
    x345 = 8*x130*x341
    x346 = x227*x331
    x347 = x328*x341
    x348 = (3/2)*x173
    x349 = (3/2)*x186
    x350 = (1/2)*x297 + (1/2)*x299 + (1/2)*x300 + (1/2)*x301 + (1/2)*x305 + (1/2)*x310 + (1/2)*x311 + x313 + (1/2)*x314 + x316 + (1/2)*x317 + x318 + (1/2)*x319 + x320 + (3/2)*x321 + (3/2)*x322 + (3/2)*x323 + (3/2)*x324 + x325*x348 + x325*x349 + x326*x348 + x326*x349
    x351 = x350*x46
    x352 = (3/2)*x42
    x353 = (1/2)*x208
    x354 = x268*x87*(20.0*r_cut*x0*x24*x48*x7 - 20.0*r_cut*x48*x85)/pow(x25, 6)
    x355 = (1/2)*x298
    x356 = (3/2)*x208
    x357 = x171*x276
    x358 = 192.0*x177
    x359 = -24.0*e*r_cut*x91*x96 + 72.0*e*x155*x16*x176 + 192.0*e*x155*x176*x95 + 24.0*e*x155*x176*x96 + 36.0*r_cut*x*x0*x22*x77 - r_cut*x358 - 72.0*r_cut*x93 + 36.0*x0*x155*x156*x24*x7 - 36.0*x157*x85 - 36.0*x180*x250
    x360 = (1/2)*x43
    x361 = (3/2)*x298
    x362 = 96.0*x73
    x363 = 96.0*x74
    x364 = 2*x88
    x365 = x280*x43
    x366 = x185*x268*x269
    x367 = x270*(-32.0*e*x15*x91*x96 + 16.0*x*x0*x15*x22*x77 + 16.0*x0*x24*x48*x7 - 16.0*x180*x78 - x267 - 32.0*x94)
    x368 = x280*x44
    x369 = x84*(288.0*e*x15*x16*x176 + 960.0*e*x15*x176*x95 + 48.0*e*x15*x176*x96 - 48.0*e*x255*x286*x96 + 120.0*x*x0*x22*x255*x256 + 24.0*x*x0*x22*x77 + 144.0*x0*x15*x156*x24*x7 - 120.0*x180*x257 - 144.0*x253*x85 - x273 - x275 - 240.0*x288 - 768.0*x289 - x358)
    x370 = x216*x35
    x371 = x218*x33
    x372 = x126*x51
    x373 = x121*x64
    x374 = (3/2)*x284
    x375 = x170*x325
    x376 = 2*x271
    x377 = x170*x326
    x378 = 2*x277
    x379 = 4*x284
    x380 = 2*x281
    return [4*r_cut*x1*x2*x228*x70 + 24*r_cut*x1*x2*x4*x69 + 12*x1*x132*x2*x70 + 24*x1*x2*x37*x4 - 12*x132*x133 - 4*x134*x228 - x328*(-x101*x230 - x101*x236 + x102*x233*x234 + x103*(x127*x292 + x160*x249 + x168*x281 + x189*x279 + x26*(-x154*x254 + x154*x258 - x157*x251 + x157*x252 + x194*x47 + x194*x50 + x250*x251 - x250*x252 - x254*x259 + x258*x259) + x260*x80 + x261*x80 + x262*x51 + x263*x51 + x264*x265 + x266*x51 + x271*x272 + x272*x285 + x278*x33 + x283*x33 + x294*x33) + x108*x231 + x108*x237 + x110*x231 + x110*x237 + x111*x231 + x111*x237 - x112*x232*x233 - x113*(x107*x260 + x107*x261 + x122*x292 + x196*x249 + x200*x281 + x205*x279 + x26*(48.0*e*x10*x155*x156*x16*x9 + 48.0*e*x155*x156*x29*x7 + 36.0*x*x0*x15*x156*x31 - x0*x254*x63 + 30.0*x0*x255*x256*x32*x7 - x0*x258*x62 + 6.0*x0*x32*x7*x77 - 48.0*x104*x250 - 48.0*x106*x250 - x195) + x262*x64 + x263*x64 + x265*x296 + x266*x64 + x271*x295 + x278*x35 + x283*x35 + x285*x295 + x294*x35) + x114*x247 + x114*x327 - x131*x247 - x131*x327 - x136*x238 - x138*x238 - x140*x240 - x140*x246 + x141*x240 + x141*x246 + x142*x240 + x142*x246 - x143*x240 - x143*x246 + x144*x240 + x144*x246 - x145*x240 - x145*x246 - x161*x68 - x163*x68 - x165*x68 - x167*x68 - x174*x68 - x184*x68 - x188*x68 - x191*x68 + x192*x235*x36 + x197*x61 + x198*x61 + x199*x61 + x201*x61 + x202*x61 + x203*x61 + x204*x61 + x206*x61 - x207*x235*x34 - x230*x81 - x230*x90 - x232*x238*x67 + x234*x238*x60 - x236*x81 - x236*x90 + x239*x65 + x239*x66 - x241*x52 - x241*x59 + x242*x65 + x242*x66 + x243*x244 + x243*x248 - x244*x245 - x245*x248) - x37*x40 - x39*x41*x69, -r_cut*x330*x332 + x134*x345 + x235*x328*x350 + x240*x347 + x244*x344 + x246*x347 + x247*x343 + x248*x344 + x327*x343 + x328*x46*(x107*x121*x364 + x107*x216*x352 + (1/2)*x122*(96.0*e*x10*x155*x156*x16*x9 + 96.0*e*x155*x156*x29*x7 + 72.0*x*x0*x15*x156*x31 + 60.0*x0*x255*x256*x32*x7 - x0*x290*x63 - x0*x291*x62 + 12.0*x0*x32*x7*x77 - 96.0*x104*x250 - 96.0*x106*x250 - x307) + x126*x364*x80 + (1/2)*x127*(-x154*x290 + x154*x291 - x157*x362 + x157*x363 + x250*x362 - x250*x363 - x259*x290 + x259*x291 + x306*x47 + x306*x50) + (1/2)*x160*x220 + x172*x359*x360 + 2*x189*x304 + (1/2)*x196*x222 + 2*x205*x309 + (1/2)*x210*x359 + 4*x216*x296 + 4*x218*x264 + x218*x352*x80 + x271*x365 + x271*x368 + x282*x370 + x282*x371 + x282*x372 + x282*x373 + x293*x370 + x293*x371 + x293*x372 + x293*x373 + (3/2)*x304*x312 + (3/2)*x309*x315 + x325*x378 + x325*x380 + x326*x378 + x326*x380 + x353*x354 + x353*x367 + x354*x355 + x355*x367 + x356*x357 + x356*x366 + x357*x361 + x360*x369 + x361*x366 + x365*x374 + x368*x374 + (1/2)*x369*x44 + x375*x376 + x375*x379 + x376*x377 + x377*x379) - x329*x330 + x329*x40 + x332*x39*x41 - x334*x335 - x334*x342 + x335*x336 + x336*x342 + x337*x338 + x337*x346 + x337*x351 - x338*x340 - x339*x345 - x340*x346 - x340*x351]


@tensorflow.function
def deriv_5(x, y, e, r_core, r_cut):
    x0 = -y
    x1 = sqrt(e)
    x2 = r_core*x1
    x3 = x0 + 2.0*x2
    x4 = e + 1
    x5 = pow(x4, -1)
    x6 = 1 - e
    x7 = x5*x6
    x8 = x*x7
    x9 = x3*x8
    x10 = -x4*y/x6
    x11 = pow(r_cut, 2)
    x12 = pow(x, 2)
    x13 = x12/pow(e + 1.0, 2) + pow(y, 2)/pow(1.0 - e, 2)
    x14 = x11 + x13
    x15 = sqrt(x14)
    x16 = 2.0*x1
    x17 = x10 + x15*x16
    x18 = x*x17 + x9
    x19 = x12*x7
    x20 = -x17*x3 + x19
    x21 = pow(x18, 2) + pow(x20, 2)
    x22 = pow(x21, -1)
    x23 = r_cut*x16
    x24 = x0 + x23
    x25 = x10 + x16*sqrt(pow(r_core, 2) + x13)
    x26 = x*x25
    x27 = x24*x8 + x26
    x28 = x19 - x24*x25
    x29 = -x18*x28 + x20*x27
    x30 = x22*x29
    x31 = x18*x27 + x20*x28
    x32 = x22*x31
    x33 = atan2(x30, x32)
    x34 = -r_core + r_cut
    x35 = pow(x1, -1)
    x36 = 0.5*pow(e, 2) - 0.5
    x37 = x35*x36
    x38 = 120*x37
    x39 = x38/pow(x34, 5)
    x40 = pow(x34, -6)
    x41 = pow(x34, -4)
    x42 = pow(x21, -2)
    x43 = pow(x31, 2)
    x44 = pow(x29, 2)
    x45 = x42*x43 + x42*x44
    x46 = pow(x45, -1)
    x47 = x*x28
    x48 = pow(x15, -1)
    x49 = x23*x48
    x50 = x27*x49
    x51 = 2.0*x*x1*x20*x5*x6 + 2.0*x1*x18*x25 - x3*x50 - x47*x49
    x52 = x22*x51
    x53 = 4.0*x1
    x54 = x48*x53
    x55 = x*x18
    x56 = x54*x55
    x57 = 4.0*r_cut*x1*x20*x3*x48 - r_cut*x56
    x58 = x29*x42
    x59 = x57*x58
    x60 = x52 + x59
    x61 = x46*x60
    x62 = x28*x3
    x63 = x*x50 + x16*x18*x8 - x16*x20*x25 - x49*x62
    x64 = x22*x63
    x65 = x31*x42
    x66 = x57*x65
    x67 = x64 + x66
    x68 = x46*x67
    x69 = -x30*x68 + x32*x61
    x70 = r_cut*x39
    x71 = x37/pow(x34, 3)
    x72 = 8.0*x48
    x73 = r_cut*x72
    x74 = e*x9
    x75 = e*x26
    x76 = x16*x48
    x77 = pow(x14, -3/2)
    x78 = x11*x77
    x79 = x16*x78
    x80 = x27*x76
    x81 = x27*x79
    x82 = -x3*x80 + x3*x81 - x47*x76 + x47*x79 - x73*x74 + x73*x75
    x83 = x22*x82
    x84 = x42*x51
    x85 = 2*x57
    x86 = x84*x85
    x87 = pow(x21, -3)
    x88 = x1*x55
    x89 = x72*x88
    x90 = 8.0*r_cut*x1*x20*x3*x48 - r_cut*x89
    x91 = x87*x90
    x92 = x57*x91
    x93 = x29*x92
    x94 = pow(x14, -1)
    x95 = e*x94
    x96 = x12*x95
    x97 = x11*x96
    x98 = pow(x2 - 0.5*y, 2)
    x99 = 4.0*x98
    x100 = x20*x3
    x101 = x53*x78
    x102 = -8.0*e*x11*x94*x99 + 4.0*x*x1*x11*x18*x77 + 4.0*x1*x20*x3*x48 - x100*x101 - x56 - 8.0*x97
    x103 = x102*x58
    x104 = x103 + x83 + x86 + x93
    x105 = x32*x46
    x106 = e*x19
    x107 = x25*x3
    x108 = e*x107
    x109 = x*x80 - x*x81 + x106*x73 + x108*x73 - x62*x76 + x62*x79
    x110 = x109*x22
    x111 = x42*x63
    x112 = x111*x85
    x113 = x31*x92
    x114 = x102*x65
    x115 = x110 + x112 + x113 + x114
    x116 = x30*x46
    x117 = pow(x45, -2)
    x118 = x43*x91
    x119 = x44*x91
    x120 = x27*x54
    x121 = x*x120
    x122 = x54*x62
    x123 = r_cut*x121 - r_cut*x122 + x18*x53*x8 - x20*x25*x53
    x124 = x123*x65
    x125 = x47*x54
    x126 = x120*x3
    x127 = -r_cut*x125 - r_cut*x126 + 4.0*x*x1*x20*x5*x6 + 4.0*x1*x18*x25
    x128 = x127*x58
    x129 = -x118 - x119 - x124 - x128
    x130 = x117*x129
    x131 = x32*x60
    x132 = x30*x67
    x133 = x104*x105 - x115*x116 + x130*x131 - x130*x132 - x52*x68 - x59*x68 + x61*x64 + x61*x66
    x134 = pow(x34, -2)
    x135 = 2*x46
    x136 = x115*x52
    x137 = x104*x64
    x138 = x104*x66
    x139 = x115*x59
    x140 = 2*x130
    x141 = x52*x67
    x142 = x60*x64
    x143 = x60*x66
    x144 = x59*x67
    x145 = x104*x32
    x146 = x115*x30
    x147 = -2*x118 - 2*x119 - 2*x124 - 2*x128
    x148 = pow(x45, -3)
    x149 = x129*x148
    x150 = x147*x149
    x151 = 12.0*x48
    x152 = e*x151
    x153 = 12.0*x78
    x154 = pow(r_cut, 3)
    x155 = pow(x14, -5/2)
    x156 = x154*x155
    x157 = 6.0*x156
    x158 = x1*x157
    x159 = x1*x27
    x160 = x159*x3
    x161 = 12.0*e*x*x11*x3*x5*x6*x77 + 12.0*e*x*x25*x48 + 6.0*r_cut*x*x1*x28*x77 + 6.0*r_cut*x1*x27*x3*x77 - x152*x9 - x153*x75 - x157*x160 - x158*x47
    x162 = x161*x22
    x163 = x42*x57
    x164 = 3*x163
    x165 = x164*x82
    x166 = 3*x92
    x167 = x166*x51
    x168 = 3*x102
    x169 = x168*x84
    x170 = x151*x88
    x171 = 12.0*r_cut*x1*x20*x3*x48 - r_cut*x170
    x172 = pow(x21, -4)
    x173 = x172*x90
    x174 = x173*x57
    x175 = x174*x29
    x176 = x171*x175
    x177 = 24.0*x96
    x178 = pow(x14, -2)
    x179 = x95*x98
    x180 = r_cut*x179
    x181 = 12.0*x156
    x182 = x1*x77
    x183 = 12.0*x182
    x184 = x100*x183
    x185 = -8.0*e*r_cut*x94*x99 + 24.0*e*x12*x154*x178 + 64.0*e*x154*x178*x98 + 8.0*e*x154*x178*x99 + 12.0*r_cut*x*x1*x18*x77 - r_cut*x177 - r_cut*x184 + 12.0*x1*x154*x155*x20*x3 - 64.0*x180 - x181*x88
    x186 = x185*x58
    x187 = x11*x182
    x188 = -16.0*e*x11*x94*x99 + 8.0*x*x1*x11*x18*x77 + 8.0*x1*x20*x3*x48 - 8.0*x100*x187 - x89 - 16.0*x97
    x189 = x188*x87
    x190 = x189*x57
    x191 = x190*x29
    x192 = x29*x91
    x193 = 2*x102
    x194 = x192*x193
    x195 = x162 + x165 + x167 + x169 + x176 + x186 + x191 + x194
    x196 = 6.0*x182
    x197 = x196*x27
    x198 = x*x197
    x199 = x*x159
    x200 = r_cut*x196*x62 - r_cut*x198 - x106*x153 + x107*x152 - x108*x153 + x152*x19 + x157*x199 - x158*x62
    x201 = x200*x22
    x202 = x109*x164
    x203 = x166*x63
    x204 = x174*x31
    x205 = x171*x204
    x206 = x111*x168
    x207 = x185*x65
    x208 = x190*x31
    x209 = x31*x91
    x210 = x193*x209
    x211 = x201 + x202 + x203 + x205 + x206 + x207 + x208 + x210
    x212 = x173*x43
    x213 = x171*x212
    x214 = x173*x44
    x215 = x171*x214
    x216 = x189*x43
    x217 = x189*x44
    x218 = 16.0*x48
    x219 = r_cut*x218
    x220 = x101*x27
    x221 = -x*x220 + x101*x62 + x106*x219 + x108*x219 + x121 - x122
    x222 = x221*x65
    x223 = x101*x47 - x125 - x126 - x219*x74 + x219*x75 + x220*x3
    x224 = x223*x58
    x225 = x127*x42
    x226 = x225*x51
    x227 = x123*x42
    x228 = x227*x63
    x229 = x123*x209
    x230 = x127*x192
    x231 = -x213 - x215 - x216 - x217 - x222 - x224 - x226 - x228 - 2*x229 - 2*x230
    x232 = x117*x231
    x233 = -x103*x68 + x105*x195 + x110*x61 + x112*x61 + x113*x61 + x114*x61 - x116*x211 + x131*x150 + x131*x232 - x132*x150 - x132*x232 - x135*x136 + x135*x137 + x135*x138 - x135*x139 - x140*x141 + x140*x142 + x140*x143 - x140*x144 + x140*x145 - x140*x146 - x68*x83 - x68*x86 - x68*x93
    x234 = r_cut*x71
    x235 = x37/x34
    x236 = 3*x46
    x237 = x115*x83
    x238 = x104*x236
    x239 = 6*x46
    x240 = x239*x57
    x241 = x115*x84
    x242 = x104*x111
    x243 = x115*x236
    x244 = 3*x67
    x245 = x130*x244
    x246 = 3*x60
    x247 = x110*x130
    x248 = 6*x130
    x249 = x248*x57
    x250 = x67*x84
    x251 = x111*x60
    x252 = x130*x246
    x253 = x244*x52
    x254 = x246*x64
    x255 = x246*x66
    x256 = x244*x59
    x257 = x211*x236
    x258 = x195*x236
    x259 = 3*x32
    x260 = 3*x30
    x261 = x131*x147
    x262 = -3*x118 - 3*x119 - 3*x124 - 3*x128
    x263 = pow(x45, -4)
    x264 = x129*x263
    x265 = x262*x264
    x266 = x132*x147
    x267 = x130*x195
    x268 = x130*x211
    x269 = x104*x232
    x270 = x115*x232
    x271 = -2*x213 - 2*x215 - 2*x216 - 2*x217 - 2*x222 - 2*x224 - 2*x226 - 2*x228 - 4*x229 - 4*x230
    x272 = x149*x271
    x273 = x148*x231
    x274 = 2*x273
    x275 = 4*x163
    x276 = x161*x275
    x277 = r_cut*x77
    x278 = 48.0*x277
    x279 = 48.0*x156
    x280 = 48.0*x75
    x281 = x1*x47
    x282 = x11*x155
    x283 = 36.0*x282
    x284 = pow(r_cut, 4)
    x285 = pow(x14, -7/2)
    x286 = x284*x285
    x287 = 30.0*x286
    x288 = x156*x280 - x160*x283 + x160*x287 + x196*x47 + x197*x3 - x277*x280 + x278*x74 - x279*x74 - x281*x283 + x281*x287
    x289 = x22*x288
    x290 = 6*x82
    x291 = x290*x92
    x292 = x102*x42
    x293 = x290*x292
    x294 = x174*x51
    x295 = 4*x171
    x296 = x294*x295
    x297 = 4*x185
    x298 = x297*x84
    x299 = x51*x91
    x300 = 8*x102
    x301 = x299*x300
    x302 = 4*x190
    x303 = x302*x51
    x304 = x171*x29
    x305 = x218*x88
    x306 = 16.0*r_cut*x1*x20*x3*x48 - r_cut*x305
    x307 = pow(x21, -5)
    x308 = x307*x90
    x309 = x306*x308
    x310 = x309*x57
    x311 = x304*x310
    x312 = 48.0*x96
    x313 = 24.0*x100*x182
    x314 = -16.0*e*r_cut*x94*x99 + 48.0*e*x12*x154*x178 + 128.0*e*x154*x178*x98 + 16.0*e*x154*x178*x99 + 24.0*r_cut*x*x1*x18*x77 - r_cut*x312 - r_cut*x313 + 24.0*x1*x154*x155*x20*x3 - 24.0*x156*x88 - 128.0*x180
    x315 = x314*x87
    x316 = x315*x57
    x317 = x29*x316
    x318 = 3*x185
    x319 = x192*x318
    x320 = -24.0*e*x11*x94*x99 + 12.0*x*x1*x11*x18*x77 + 12.0*x1*x20*x3*x48 - x11*x177 - x11*x184 - x170
    x321 = x29*x320
    x322 = x174*x321
    x323 = x168*x173
    x324 = x304*x323
    x325 = x172*x188
    x326 = x325*x85
    x327 = x304*x326
    x328 = e*x12
    x329 = pow(x14, -3)
    x330 = x284*x329
    x331 = x328*x330
    x332 = e*x98
    x333 = x330*x332
    x334 = 72.0*x282
    x335 = x1*x100
    x336 = 60.0*x286
    x337 = 144.0*e*x11*x12*x178 + 480.0*e*x11*x178*x98 + 24.0*e*x11*x178*x99 - 24.0*e*x284*x329*x99 + 60.0*x*x1*x18*x284*x285 + 12.0*x*x1*x18*x77 + 72.0*x1*x11*x155*x20*x3 - x177 - 96.0*x179 - x184 - 120.0*x331 - 384.0*x333 - x334*x88 - x335*x336
    x338 = x337*x58
    x339 = x168*x189
    x340 = x29*x339
    x341 = x276 + x289 + x291 + x293 + x296 + x298 + x301 + x303 + x311 + x317 + x319 + x322 + x324 + x327 + x338 + x340
    x342 = x200*x275
    x343 = x1*x62
    x344 = 48.0*e*x12*x154*x155*x5*x6 + 48.0*e*x154*x155*x25*x3 + 36.0*x*x1*x11*x155*x27 + 30.0*x1*x28*x284*x285*x3 + 6.0*x1*x28*x3*x77 - x106*x278 - 48.0*x108*x277 - x198 - x199*x287 - x283*x343
    x345 = x22*x344
    x346 = 6*x109
    x347 = x346*x92
    x348 = x292*x346
    x349 = x174*x63
    x350 = x295*x349
    x351 = x111*x297
    x352 = x171*x31
    x353 = x310*x352
    x354 = x63*x91
    x355 = x300*x354
    x356 = x302*x63
    x357 = x31*x316
    x358 = x209*x318
    x359 = x204*x320
    x360 = x323*x352
    x361 = x326*x352
    x362 = x337*x65
    x363 = x31*x339
    x364 = x342 + x345 + x347 + x348 + x350 + x351 + x353 + x355 + x356 + x357 + x358 + x359 + x360 + x361 + x362 + x363
    x365 = x171*x43
    x366 = x306*x365
    x367 = x308*x366
    x368 = x171*x44
    x369 = x308*x368
    x370 = x306*x369
    x371 = x315*x43
    x372 = x315*x44
    x373 = 24.0*x48
    x374 = 24.0*x78
    x375 = 24.0*e*x*x11*x3*x5*x6*x77 + 24.0*e*x*x25*x48 + 12.0*r_cut*x*x1*x28*x77 + 12.0*r_cut*x1*x27*x3*x77 - x160*x181 - x181*x281 - x373*x74 - x374*x75
    x376 = x375*x58
    x377 = x183*x27
    x378 = x*x377
    x379 = r_cut*x183*x62 - r_cut*x378 + x106*x373 - x106*x374 + x108*x373 - x108*x374 + x181*x199 - x181*x343
    x380 = x379*x65
    x381 = x225*x82
    x382 = x223*x84
    x383 = x109*x227
    x384 = x111*x221
    x385 = x320*x43
    x386 = x173*x385
    x387 = x325*x365
    x388 = x214*x320
    x389 = x325*x368
    x390 = x209*x221
    x391 = x192*x223
    x392 = x127*x299
    x393 = x123*x354
    x394 = x123*x31
    x395 = x171*x173
    x396 = 3*x395
    x397 = x127*x29
    x398 = 3*x189
    x399 = -x367 - x370 - x371 - x372 - x376 - x380 - x381 - 2*x382 - x383 - 2*x384 - x386 - 2*x387 - x388 - 2*x389 - 3*x390 - 3*x391 - 3*x392 - 3*x393 - x394*x396 - x394*x398 - x396*x397 - x397*x398
    x400 = x117*x399
    x401 = -x103*x243 - x103*x245 + x104*x150*x259 + x105*x341 + x110*x238 + x113*x238 + x113*x252 + x114*x238 + x114*x252 - x115*x150*x260 - x116*x364 + x131*x272 + x131*x400 - x132*x272 - x132*x400 - x136*x248 + x137*x248 + x138*x248 - x139*x248 - x150*x253 + x150*x254 + x150*x255 - x150*x256 - x162*x68 - x165*x68 - x167*x68 - x169*x68 - x176*x68 - x186*x68 - x191*x68 - x194*x68 + x201*x61 + x202*x61 + x203*x61 + x205*x61 + x206*x61 + x207*x61 + x208*x61 + x210*x61 - x232*x253 + x232*x254 + x232*x255 - x232*x256 - x236*x237 - x240*x241 + x240*x242 - x243*x93 - x245*x83 - x245*x93 + x246*x247 - x249*x250 + x249*x251 - x257*x52 - x257*x59 + x258*x64 + x258*x66 + x259*x267 + x259*x269 - x260*x268 - x260*x270 + x261*x265 + x261*x274 - x265*x266 - x266*x274
    x402 = 4*x46
    x403 = x115*x402
    x404 = x104*x402
    x405 = x163*x82
    x406 = 12*x46
    x407 = x115*x406
    x408 = x104*x406
    x409 = x109*x163
    x410 = x51*x92
    x411 = x63*x92
    x412 = x102*x406
    x413 = x300*x46
    x414 = 4*x130
    x415 = x414*x67
    x416 = x414*x60
    x417 = 12*x130
    x418 = x417*x67
    x419 = x417*x60
    x420 = x102*x417
    x421 = x130*x300
    x422 = 24*x130*x57
    x423 = x104*x417
    x424 = x115*x417
    x425 = x67*x83
    x426 = 6*x150
    x427 = x110*x60
    x428 = 12*x150
    x429 = x428*x57
    x430 = x426*x60
    x431 = x426*x67
    x432 = x211*x239
    x433 = x195*x239
    x434 = x406*x57
    x435 = x141*x147
    x436 = 4*x265
    x437 = x147*x436
    x438 = 6*x232
    x439 = 12*x232
    x440 = x439*x57
    x441 = x438*x60
    x442 = x438*x67
    x443 = 12*x268
    x444 = 12*x267
    x445 = x129*x262*(-4*x118 - 4*x119 - 4*x124 - 4*x128)/pow(x45, 5)
    x446 = 8*x273
    x447 = 4*x272
    x448 = x147*x446
    x449 = x195*x32
    x450 = x211*x30
    x451 = x264*(-3*x213 - 3*x215 - 3*x216 - 3*x217 - 3*x222 - 3*x224 - 3*x226 - 3*x228 - 6*x229 - 6*x230)
    x452 = 3*x261
    x453 = x231*x262*x263
    x454 = x131*x271
    x455 = 2*x265
    x456 = 3*x266
    x457 = x132*x271
    x458 = x364*x402
    x459 = x341*x402
    x460 = 3*x273
    x461 = 4*x400
    x462 = 6*x395
    x463 = 6*x189
    x464 = x149*(-2*x367 - 2*x370 - 2*x371 - 2*x372 - 2*x376 - 2*x380 - 2*x381 - 4*x382 - 2*x383 - 4*x384 - 2*x386 - 4*x387 - 2*x388 - 4*x389 - 6*x390 - 6*x391 - 6*x392 - 6*x393 - x394*x462 - x394*x463 - x397*x462 - x397*x463)
    x465 = x148*x399
    x466 = 60.0*x77
    x467 = 300.0*x286
    x468 = x154*x285
    x469 = pow(r_cut, 5)
    x470 = pow(x14, -9/2)
    x471 = r_cut*x155
    x472 = 90.0*x471
    x473 = x469*x470
    x474 = 210.0*x473
    x475 = 10*x92
    x476 = 5*x163
    x477 = 10*x292
    x478 = 10*x171
    x479 = x174*x478
    x480 = 10*x185
    x481 = x109*x42
    x482 = 20*x102
    x483 = x109*x91
    x484 = 10*x190
    x485 = 5*x171
    x486 = x310*x485
    x487 = r_cut*x178
    x488 = x328*x487
    x489 = x154*x329
    x490 = x328*x489
    x491 = pow(x14, -4)
    x492 = x469*x491
    x493 = x328*x492
    x494 = x332*x487
    x495 = x332*x489
    x496 = x332*x492
    x497 = 180.0*x471
    x498 = 600.0*x468
    x499 = 420.0*x473
    x500 = 24.0*e*r_cut*x178*x99 - 144.0*e*x154*x329*x99 + 120.0*e*x469*x491*x99 + x335*x497 - x335*x498 + x335*x499 + 360.0*x488 - 1200.0*x490 + 840.0*x493 + 1344.0*x494 - 4224.0*x495 + 2880.0*x496 - x497*x88 + x498*x88 - x499*x88
    x501 = 15*x185
    x502 = 5*x316
    x503 = 20.0*x48*x88
    x504 = 20.0*r_cut*x1*x20*x3*x48 - r_cut*x503
    x505 = pow(x21, -6)
    x506 = x505*x90
    x507 = x504*x506
    x508 = x352*x57
    x509 = x306*x508
    x510 = 15*x102
    x511 = x395*x510
    x512 = x325*x478
    x513 = x512*x57
    x514 = 5*x320
    x515 = 5*x337
    x516 = 72.0*x96
    x517 = 192.0*x179
    x518 = 36.0*x100*x182
    x519 = -24.0*e*r_cut*x94*x99 + 72.0*e*x12*x154*x178 + 192.0*e*x154*x178*x98 + 24.0*e*x154*x178*x99 + 36.0*r_cut*x*x1*x18*x77 - r_cut*x516 - r_cut*x517 - r_cut*x518 + 36.0*x1*x154*x155*x20*x3 - 36.0*x156*x88
    x520 = 6*x173*x185
    x521 = x172*x314
    x522 = 3*x521
    x523 = -32.0*e*x11*x94*x99 + 16.0*x*x1*x11*x18*x77 + 16.0*x1*x20*x3*x48 - 16.0*x100*x187 - x305 - 32.0*x97
    x524 = x308*x523
    x525 = x189*x63
    x526 = x102*x309
    x527 = x188*x307
    x528 = 3*x527
    x529 = x31*x320
    x530 = x309*x85
    x531 = 288.0*e*x11*x12*x178 + 960.0*e*x11*x178*x98 + 48.0*e*x11*x178*x99 - 48.0*e*x284*x329*x99 + 120.0*x*x1*x18*x284*x285 + 24.0*x*x1*x18*x77 + 144.0*x1*x11*x155*x20*x3 - 144.0*x282*x88 - 120.0*x286*x335 - x312 - x313 - 240.0*x331 - 768.0*x333 - x517
    x532 = x531*x87
    x533 = x532*x57
    x534 = 4*x337
    x535 = 4*x315
    x536 = x102*x535
    x537 = x185*x463
    x538 = x300*x325
    x539 = 4*x173
    x540 = x102*x539
    x541 = 3*x325
    x542 = x541*x57
    x543 = x42*x82
    x544 = x82*x91
    x545 = x304*x57
    x546 = x306*x545
    x547 = x189*x51
    x548 = x161*x225
    x549 = x375*x84
    x550 = x200*x227
    x551 = x111*x379
    x552 = x223*x543
    x553 = x221*x481
    x554 = x366*x507
    x555 = x306*x368
    x556 = x507*x555
    x557 = x212*x519
    x558 = x214*x519
    x559 = x192*x375
    x560 = x209*x379
    x561 = 96.0*r_cut
    x562 = x561*x77
    x563 = 96.0*x156
    x564 = -x160*x334 + x160*x336 + x183*x47 - x281*x334 + x281*x336 + x3*x377 + x562*x74 - x562*x75 - x563*x74 + x563*x75
    x565 = x564*x58
    x566 = 96.0*e*x12*x154*x155*x5*x6 + 96.0*e*x154*x155*x25*x3 + 72.0*x*x1*x11*x155*x27 + 60.0*x1*x28*x284*x285*x3 + 12.0*x1*x28*x3*x77 - x106*x562 - x108*x562 - x199*x336 - x334*x343 - x378
    x567 = x566*x65
    x568 = x127*x544
    x569 = x223*x299
    x570 = x221*x354
    x571 = x123*x483
    x572 = x365*x524
    x573 = x309*x385
    x574 = x369*x523
    x575 = x320*x44
    x576 = x309*x575
    x577 = x43*x532
    x578 = x44*x532
    x579 = x221*x31
    x580 = x223*x29
    x581 = x127*x51
    x582 = x123*x63
    x583 = x295*x309
    x584 = x325*x394
    x585 = 8*x171
    x586 = x320*x394
    x587 = x325*x397
    x588 = x127*x321
    x589 = x117*(-x365*x522 - x366*x528 - x368*x522 - x385*x541 - x394*x535 - x394*x583 - x397*x535 - x397*x583 - x462*x579 - x462*x580 - x462*x581 - x462*x582 - x463*x579 - x463*x580 - x463*x581 - x463*x582 - x528*x555 - x539*x586 - x539*x588 - x541*x575 - x548 - 3*x549 - x550 - 3*x551 - 3*x552 - 3*x553 - x554 - x556 - x557 - x558 - 4*x559 - 4*x560 - x565 - x567 - 4*x568 - 8*x569 - 8*x570 - 4*x571 - x572 - 2*x573 - x574 - 2*x576 - x577 - x578 - x584*x585 - x585*x587)
    x590 = r_cut*x235
    x591 = log(sqrt(x45))
    x592 = (1/2)*x118 + (1/2)*x119 + (1/2)*x124 + (1/2)*x128
    x593 = x46*x592
    x594 = 60*x71
    x595 = x130*x592
    x596 = 60*r_cut*x37*x41
    x597 = x134*x37
    x598 = 20*x597
    x599 = x592*x598
    x600 = 20*x234
    x601 = x592*x600
    x602 = (1/2)*x213 + (1/2)*x215 + (1/2)*x216 + (1/2)*x217 + (1/2)*x222 + (1/2)*x224 + (1/2)*x226 + (1/2)*x228 + x229 + x230
    x603 = x46*x602
    x604 = 5*x235
    x605 = x147*x592
    x606 = x265*x605
    x607 = r_cut*x597
    x608 = 5*x607
    x609 = 40*x130*x602
    x610 = x590*x592
    x611 = x147*x610
    x612 = 15*x235
    x613 = x150*x602
    x614 = 15*x607
    x615 = 10*x273*x605
    x616 = x272*x592
    x617 = x590*x602
    x618 = 3*x611
    x619 = x271*x610
    x620 = x232*x602
    x621 = (3/2)*x395
    x622 = (3/2)*x189
    x623 = (1/2)*x367 + (1/2)*x370 + (1/2)*x371 + (1/2)*x372 + (1/2)*x376 + (1/2)*x380 + (1/2)*x381 + x382 + (1/2)*x383 + x384 + (1/2)*x386 + x387 + (1/2)*x388 + x389 + (3/2)*x390 + (3/2)*x391 + (3/2)*x392 + (3/2)*x393 + x394*x621 + x394*x622 + x397*x621 + x397*x622
    x624 = x46*x623
    x625 = x130*x623
    x626 = x400*x592
    x627 = x590*x623
    x628 = (3/2)*x521
    x629 = (3/2)*x527
    x630 = (3/2)*x325
    x631 = x171*x394
    x632 = 2*x309
    x633 = x171*x397
    x634 = 2*x315
    x635 = 2*x173
    x636 = x295*x584 + x295*x587 + x365*x628 + x366*x629 + x368*x628 + x385*x630 + x394*x634 + x396*x579 + x396*x580 + x396*x581 + x396*x582 + x397*x634 + x398*x579 + x398*x580 + x398*x581 + x398*x582 + (1/2)*x548 + (3/2)*x549 + (1/2)*x550 + (3/2)*x551 + (3/2)*x552 + (3/2)*x553 + (1/2)*x554 + x555*x629 + (1/2)*x556 + (1/2)*x557 + (1/2)*x558 + 2*x559 + 2*x560 + (1/2)*x565 + (1/2)*x567 + 2*x568 + 4*x569 + 4*x570 + 2*x571 + (1/2)*x572 + x573 + (1/2)*x574 + x575*x630 + x576 + (1/2)*x577 + (1/2)*x578 + x586*x635 + x588*x635 + x631*x632 + x632*x633
    x637 = x46*x636
    x638 = 2*x42
    x639 = 360.0*x471
    x640 = 1200.0*x468
    x641 = 840.0*x473
    x642 = (1/2)*x87*(48.0*e*r_cut*x178*x99 - 288.0*e*x154*x329*x99 + 240.0*e*x469*x491*x99 + x335*x639 - x335*x640 + x335*x641 + 720.0*x488 - 2400.0*x490 + 1680.0*x493 + 2688.0*x494 - 8448.0*x495 + 5760.0*x496 - x639*x88 + x640*x88 - x641*x88)
    x643 = 120.0*x77
    x644 = 600.0*x286
    x645 = (5/2)*x91
    x646 = (1/2)*x366
    x647 = x504*x90*(24.0*r_cut*x1*x20*x3*x48 - r_cut*x373*x88)/pow(x21, 7)
    x648 = (1/2)*x555
    x649 = 3*x307*x314
    x650 = x43*x519
    x651 = (3/2)*x309
    x652 = -16.0*e*r_cut*x94*x99 + 48.0*e*x12*x154*x178 + 128.0*e*x154*x178*x98 + 16.0*e*x154*x178*x99 + 24.0*r_cut*x*x1*x18*x77 - 24.0*r_cut*x100*x182 + 24.0*x1*x154*x155*x20*x3 - 128.0*x180 - 1/2*x279*x88 - 1/2*x561*x96
    x653 = x44*x519
    x654 = x29*x375
    x655 = 5*x395
    x656 = x31*x379
    x657 = x127*x82
    x658 = 10*x223
    x659 = 10*x221
    x660 = x109*x123
    x661 = x507*x523
    x662 = x188*x504*x505
    x663 = x306*x385
    x664 = (3/2)*x507
    x665 = x506*(-40.0*e*x11*x94*x99 + 20.0*x*x1*x11*x18*x77 + 20.0*x1*x20*x3*x48 - 20.0*x100*x187 - x503 - 40.0*x97)
    x666 = 2*x368
    x667 = x306*x575
    x668 = 2*x365
    x669 = x172*x531
    x670 = 216.0*e*x11*x12*x178 + 720.0*e*x11*x178*x98 + 36.0*e*x11*x178*x99 - 36.0*e*x284*x329*x99 + 90.0*x*x1*x18*x284*x285 + 18.0*x*x1*x18*x77 + 108.0*x1*x11*x155*x20*x3 - 144.0*x179 - 108.0*x282*x88 - 90.0*x286*x335 - 180.0*x331 - 576.0*x333 - 1/2*x516 - 1/2*x518
    x671 = 2*x325
    x672 = 5*x189
    x673 = x309*x485
    x674 = 5*x315
    x675 = 4*x527
    x676 = x523*x527
    x677 = (3/2)*x524
    x678 = x173*x514
    x679 = (5/2)*x394
    x680 = x171*x306*x507
    x681 = (5/2)*x397
    x682 = (15/2)*x521
    x683 = x173*x519
    x684 = (15/2)*x306*x527
    x685 = x171*x524
    x686 = (5/2)*x532
    return [60*r_cut*x133*x35*x36*x41 + 5*r_cut*x134*x35*x36*x401 + 120*r_cut*x33*x35*x36*x40 - 60*x133*x71 + 20*x134*x233*x35*x36 - 20*x233*x234 - 5*x235*x401 - x33*x39 + 120*x35*x36*x41*x69 - x590*(-x103*x424 - x103*x431 - x103*x432 - x103*x442 + x104*x209*x413 + 12*x104*x247 + x105*(x161*x475 + x161*x477 + x175*x519 + x192*x534 + x22*(360.0*e*x*x11*x155*x25 + 300.0*e*x*x284*x285*x3*x5*x6 + 60.0*e*x*x3*x5*x6*x77 + 300.0*x*x1*x154*x28*x285 + 300.0*x1*x154*x27*x285*x3 - x160*x472 - x160*x474 - x281*x472 - x281*x474 - 360.0*x282*x74 - x466*x75 - x467*x75) + x288*x476 + x29*x533 + x29*x536 + x29*x537 + x294*x514 + x299*x501 + x304*x520 + 4*x304*x526 + x304*x538 + x321*x530 + x321*x540 + x321*x542 + x479*x82 + x480*x543 + x482*x544 + x484*x82 + x486*x51 + x500*x58 + x502*x51 + x507*x546 + x51*x511 + x51*x513 + x510*x547 + x515*x84 + x522*x545 + x524*x545 + x528*x546) + x110*x433 + x111*x195*x434 + x113*x423 + x113*x430 + x113*x433 + x113*x441 + x114*x423 + x114*x430 + x114*x433 + x114*x441 - x115*x192*x413 - x116*(x109*x479 + x109*x484 + x111*x515 + x200*x475 + x200*x477 + x204*x519 + x209*x534 + x22*(360.0*e*x11*x12*x155*x5*x6 + 360.0*e*x11*x155*x25*x3 + 90.0*r_cut*x*x1*x155*x27 + 210.0*x*x1*x27*x469*x470 + 300.0*x1*x154*x28*x285*x3 - x106*x466 - x106*x467 - x108*x466 - x108*x467 - 300.0*x199*x468 - x343*x472 - x343*x474) + x295*x31*x526 + x31*x533 + x31*x536 + x31*x537 + x344*x476 + x349*x514 + x352*x520 + x352*x538 + x354*x501 + x480*x481 + x482*x483 + x486*x63 + x500*x65 + x502*x63 + x507*x509 + x508*x522 + x508*x524 + x509*x528 + x510*x525 + x511*x63 + x513*x63 + x529*x530 + x529*x540 + x529*x542) + x131*x464 + x131*x589 - x132*x464 - x132*x589 - x136*x428 - x136*x439 + x137*x428 + x137*x439 + x138*x428 - x139*x428 - x141*x447 - x141*x461 + x142*x437 + x142*x447 + x142*x448 + x142*x461 + x143*x437 + x143*x447 + x143*x448 + x143*x461 - x144*x437 - x144*x447 - x144*x448 - x144*x461 + x145*x437 + x145*x447 + x145*x448 + x145*x461 - x146*x437 - x146*x447 - x146*x448 - x146*x461 - x162*x403 - x162*x415 - x176*x403 - x176*x415 - x186*x403 - x186*x415 - x191*x403 - x191*x415 - x192*x421*x67 + x201*x404 + x201*x416 + x205*x404 + x205*x416 + x207*x404 + x207*x416 + x208*x404 + x208*x416 + x209*x421*x60 - x211*x434*x84 - x237*x417 - x241*x412 - x241*x422 + x242*x412 + x242*x422 - x250*x420 - x250*x429 - x250*x440 + x251*x420 + x251*x429 + x251*x440 + x261*x445 + x261*x451 - x266*x445 - x266*x451 + 12*x269*x66 - 12*x270*x59 - x276*x68 - x289*x68 - x291*x68 - x293*x68 - x296*x68 - x298*x68 - x30*x364*x414 - x301*x68 - x303*x68 - x311*x68 - x317*x68 - x319*x68 + x32*x341*x414 - x322*x68 - x324*x68 - x327*x68 - x338*x68 - x340*x68 + x342*x61 + x345*x61 + x347*x61 + x348*x61 + x350*x61 + x351*x61 + x353*x61 + x355*x61 + x356*x61 + x357*x61 + x358*x61 + x359*x61 + x360*x61 + x361*x61 + x362*x61 + x363*x61 - x405*x407 - x405*x418 - x407*x410 + x408*x409 + x408*x411 + x409*x419 - x410*x418 + x411*x419 - x424*x93 - x425*x426 - x425*x438 + x426*x427 + x426*x449 - x426*x450 + x427*x438 - x431*x93 - x432*x83 - x432*x93 - x435*x436 - x435*x446 + x438*x449 - x438*x450 - x442*x93 - x443*x52 - x443*x59 + x444*x64 + x444*x66 + x452*x453 + x452*x465 - x453*x456 + x454*x455 + x454*x460 - x455*x457 - x456*x465 - x457*x460 - x458*x52 - x458*x59 + x459*x64 + x459*x66) - x69*x70, -r_cut*x38*x40*x591 - x150*x599 + x150*x601 - x232*x599 + x232*x601 + x234*x609 + x235*x615 - x38*x41*x593 + x39*x591 + x414*x590*x636 + x426*x627 + x437*x617 + x438*x627 + x445*x611 + x447*x617 + x448*x617 + x451*x611 + x453*x618 + x455*x619 + x46*x590*(2*x111*x566 + x123*x200*x645 + x127*x161*x645 + x161*x223*x638 + 5*x173*x223*x321 + (5/2)*x192*x564 + x200*x221*x638 + (5/2)*x209*x566 + x212*x670 + x214*x670 + (15/2)*x221*x483 + (15/2)*x223*x544 + (1/2)*x225*x288 + (1/2)*x227*x344 + (15/2)*x299*x375 + x306*x662*x666 + x308*x365*x652 + x309*x394*x514 + 5*x309*x588 + (15/2)*x320*x584 + (15/2)*x325*x588 + (15/2)*x354*x379 + x365*x661 + x366*x649 + 2*x366*x662 + x368*x661 + x369*x652 + 3*x375*x543 + 3*x379*x481 + x385*x522 + x385*x677 + x394*x686 + x395*x51*x658 + x395*x63*x659 + x397*x686 + x43*x642 + x44*x642 + x512*x579 + x512*x580 + x512*x581 + x512*x582 + x522*x575 + x525*x659 + x547*x658 + x555*x649 + 2*x564*x84 + x575*x677 + x579*x673 + x579*x674 + x579*x678 + (1/2)*x58*(720.0*e*x*x11*x155*x25 + 600.0*e*x*x284*x285*x3*x5*x6 + 120.0*e*x*x3*x5*x6*x77 + 600.0*x*x1*x154*x28*x285 + 600.0*x1*x154*x27*x285*x3 - x160*x497 - x160*x499 - x281*x497 - x281*x499 - 720.0*x282*x74 - x643*x75 - x644*x75) + x580*x673 + x580*x674 + x581*x673 + x581*x674 + x581*x678 + x582*x673 + x582*x674 + x582*x678 + x631*x682 + x631*x684 + x633*x682 + x633*x684 + x646*x647 + x646*x665 + x647*x648 + x648*x665 + (1/2)*x65*(720.0*e*x11*x12*x155*x5*x6 + 720.0*e*x11*x155*x25*x3 + 180.0*r_cut*x*x1*x155*x27 + 420.0*x*x1*x27*x469*x470 + 600.0*x1*x154*x28*x285*x3 - x106*x643 - x106*x644 - x108*x643 - x108*x644 - x199*x498 - x343*x497 - x343*x499) + x650*x651 + x650*x671 + x651*x653 + x653*x671 + x654*x655 + x654*x672 + x655*x656 + x655*x657 + x655*x660 + x656*x672 + x657*x672 + x660*x672 + x663*x664 + x663*x675 + x664*x667 + x666*x669 + x666*x676 + x667*x675 + x668*x669 + x668*x676 + x679*x680 + x679*x683 + x679*x685 + x680*x681 + x681*x683 + x681*x685) + x460*x619 + x461*x617 + x464*x610 + x465*x618 + x589*x610 + x593*x70 + x594*x595 + x594*x603 - x595*x596 - x596*x603 - x597*x609 - x598*x624 + x600*x624 + x604*x606 + x604*x616 + x604*x626 + x604*x637 - x606*x608 - x607*x615 - x608*x616 - x608*x626 - x608*x637 + x612*x613 + x612*x620 + x612*x625 - x613*x614 - x614*x620 - x614*x625]


@tensorflow.function
def hessian_0(x, y, e, r_core, r_cut):
    x0 = -y
    x1 = sqrt(e)
    x2 = r_core*x1
    x3 = pow(x, 2)
    x4 = 0.5*y
    x5 = -x4
    x6 = pow(x3 + 4.0*pow(x2 + x5, 2), -1)
    x7 = x6*(x0 + 2.0*x2)
    x8 = e + 1
    x9 = pow(x8, 2)
    x10 = 1 - e
    x11 = pow(x10, 2)
    x12 = x11*x3/x9
    x13 = x8/x10
    x14 = -x13*x4
    x15 = x3/pow(e + 1.0, 2)
    x16 = pow(1.0 - e, -2)
    x17 = x16*pow(y, 2)
    x18 = x15 + x17
    x19 = sqrt(pow(r_core, 2) + x18)
    x20 = x1*x19
    x21 = pow(x12 + 4.0*pow(x14 + x20, 2), -1)
    x22 = 1.0*y
    x23 = x13*x22
    x24 = 2.0*x20
    x25 = pow(x19, -1)
    x26 = 2.0*x1
    x27 = x15*x26
    x28 = x10/x8
    x29 = (0.5 - 0.5*pow(e, 2))/x1
    x30 = r_cut*x1
    x31 = pow(x3 + 4.0*pow(x30 + x5, 2), -1)
    x32 = x31*(x0 + 2.0*x30)
    x33 = sqrt(pow(r_cut, 2) + x18)
    x34 = x1*x33
    x35 = pow(x12 + 4.0*pow(x14 + x34, 2), -1)
    x36 = 2.0*x34
    x37 = pow(x33, -1)
    x38 = r_cut/(-r_core + r_cut)
    x39 = -x
    x40 = x16*y
    x41 = 2*x*x1*x28*x40
    x42 = x38*(-x29*(x*x31 + x35*(x37*x41 + x39)) + x29*(x*x6 + x21*(x25*x41 + x39)))
    x43 = x13*x17*x26
    x44 = -4*e*x40 - x22*x9/x11
    return [x38*(x29*(x21*x28*(x23 - x24 + x25*x27) + x7) - x29*(x28*x35*(x23 + x27*x37 - x36) + x32)), x42, x42, x38*(-x29*(-x32 + x35*(x13*x36 + x37*x43 + x44)) + x29*(x21*(x13*x24 + x25*x43 + x44) - x7))]


@tensorflow.function
def hessian_1(x, y, e, r_core, r_cut):
    x0 = -r_core + r_cut
    x1 = pow(x0, -1)
    x2 = -y
    x3 = sqrt(e)
    x4 = r_core*x3
    x5 = pow(x, 2)
    x6 = 0.5*y
    x7 = -x6
    x8 = pow(x5 + 4.0*pow(x4 + x7, 2), -1)
    x9 = x8*(x2 + 2.0*x4)
    x10 = e + 1
    x11 = pow(x10, 2)
    x12 = 1 - e
    x13 = pow(x12, 2)
    x14 = x13/x11
    x15 = x14*x5
    x16 = pow(x12, -1)
    x17 = x10*x16
    x18 = -x17*x6
    x19 = x5/pow(e + 1.0, 2)
    x20 = pow(1.0 - e, -2)
    x21 = x20*pow(y, 2)
    x22 = x19 + x21
    x23 = sqrt(pow(r_core, 2) + x22)
    x24 = x23*x3
    x25 = pow(x15 + 4.0*pow(x18 + x24, 2), -1)
    x26 = 1.0*y
    x27 = x17*x26
    x28 = 2.0*x24
    x29 = pow(x23, -1)
    x30 = 2.0*x3
    x31 = x19*x30
    x32 = pow(x10, -1)
    x33 = x12*x32
    x34 = (0.5 - 0.5*pow(e, 2))/x3
    x35 = r_cut*x3
    x36 = 2.0*x35
    x37 = x2 + x36
    x38 = x35 + x7
    x39 = pow(x38, 2)
    x40 = pow(4.0*x39 + x5, -1)
    x41 = x37*x40
    x42 = pow(r_cut, 2) + x22
    x43 = sqrt(x42)
    x44 = x3*x43
    x45 = 2.0*x44
    x46 = pow(x43, -1)
    x47 = x27 + x31*x46 - x45
    x48 = x18 + x44
    x49 = pow(x48, 2)
    x50 = pow(x15 + 4.0*x49, -1)
    x51 = x33*x50
    x52 = -x34*(x41 + x47*x51) + x34*(x25*x33*(x27 - x28 + x29*x31) + x9)
    x53 = r_cut/pow(x0, 2)
    x54 = x30*x40
    x55 = 0.25*x5
    x56 = 0.5*x3*x38/pow(x39 + x55, 2)
    x57 = x37*x56
    x58 = pow(x42, -3/2)
    x59 = x36*x58
    x60 = 0.5*x35*x46*x48/pow(x14*x55 + x49, 2)
    x61 = r_cut*x1*x34
    x62 = -x
    x63 = x20*y
    x64 = 2*x*x63
    x65 = x3*x33*x64
    x66 = x46*x65 + x62
    x67 = -x34*(x*x40 + x50*x66) + x34*(x*x8 + x25*(x29*x65 + x62))
    x68 = x1*x67 - x53*x67 - x61*(-x*x56 - x35*x51*x58*x64 - x60*x66)
    x69 = x17*x21
    x70 = x30*x69
    x71 = -4*e*x63 - x11*x26/x13
    x72 = x17*x45 + x46*x70 + x71
    x73 = -x34*(-x41 + x50*x72) + x34*(x25*(x17*x28 + x29*x70 + x71) - x9)
    return [x1*x52 - x52*x53 - x61*(x12*x32*x50*(-x19*x59 - x36*x46) - x33*x47*x60 + x54 - x57), x68, x68, x1*x73 - x53*x73 - x61*(x50*(2.0*r_cut*x10*x16*x3*x46 - x59*x69) - x54 + x57 - x60*x72)]


@tensorflow.function
def hessian_2(x, y, e, r_core, r_cut):
    x0 = -y
    x1 = sqrt(e)
    x2 = r_core*x1
    x3 = pow(x, 2)
    x4 = 0.5*y
    x5 = -x4
    x6 = pow(x3 + 4.0*pow(x2 + x5, 2), -1)
    x7 = x6*(x0 + 2.0*x2)
    x8 = e + 1
    x9 = pow(x8, 2)
    x10 = 1 - e
    x11 = pow(x10, 2)
    x12 = x11/x9
    x13 = x12*x3
    x14 = pow(x10, -1)
    x15 = x14*x8
    x16 = -x15*x4
    x17 = x3/pow(e + 1.0, 2)
    x18 = pow(1.0 - e, -2)
    x19 = x18*pow(y, 2)
    x20 = x17 + x19
    x21 = sqrt(pow(r_core, 2) + x20)
    x22 = x1*x21
    x23 = pow(x13 + 4.0*pow(x16 + x22, 2), -1)
    x24 = 1.0*y
    x25 = x15*x24
    x26 = 2.0*x22
    x27 = pow(x21, -1)
    x28 = 2.0*x1
    x29 = x27*x28
    x30 = pow(x8, -1)
    x31 = x10*x30
    x32 = pow(x1, -1)
    x33 = 0.5 - 0.5*pow(e, 2)
    x34 = x32*x33
    x35 = r_cut*x1
    x36 = 2.0*x35
    x37 = x0 + x36
    x38 = x35 + x5
    x39 = pow(x38, 2)
    x40 = pow(x3 + 4.0*x39, -1)
    x41 = x37*x40
    x42 = pow(r_cut, 2)
    x43 = x20 + x42
    x44 = sqrt(x43)
    x45 = x1*x44
    x46 = 2.0*x45
    x47 = pow(x44, -1)
    x48 = x28*x47
    x49 = x17*x48 + x25 - x46
    x50 = x16 + x45
    x51 = pow(x50, 2)
    x52 = pow(x13 + 4.0*x51, -1)
    x53 = x31*x52
    x54 = -x34*(x41 + x49*x53) + x34*(x23*x31*(x17*x29 + x25 - x26) + x7)
    x55 = -r_core + r_cut
    x56 = pow(x55, -2)
    x57 = 2*x56
    x58 = pow(x55, -3)
    x59 = x28*x40
    x60 = 0.25*x3
    x61 = x39 + x60
    x62 = pow(x61, -2)
    x63 = 0.5*x1
    x64 = x38*x62*x63
    x65 = x37*x64
    x66 = pow(x43, -3/2)
    x67 = x36*x66
    x68 = -x17*x67 - x36*x47
    x69 = x12*x60 + x51
    x70 = pow(x69, -2)
    x71 = x47*x50
    x72 = x35*x71
    x73 = x70*x72
    x74 = x31*x49
    x75 = x10*x30*x52*x68 + x59 - x65 - 0.5*x73*x74
    x76 = x34/x55
    x77 = 2*x76
    x78 = x28*x66
    x79 = x42*x78
    x80 = x1*x42/pow(x43, 5/2)
    x81 = 6.0*x80
    x82 = pow(x43, -1)
    x83 = x42*x82
    x84 = e*x83
    x85 = x70*x74
    x86 = x63*x71
    x87 = 1.0*x73
    x88 = pow(x69, -3)
    x89 = e*x62
    x90 = 0.5*x89
    x91 = 2.0*e
    x92 = x39*x91/pow(x61, 3)
    x93 = x37*x90 - x37*x92 + 2.0*x38*x89
    x94 = r_cut*x76
    x95 = -x
    x96 = x18*y
    x97 = x*x96
    x98 = 2*x97
    x99 = x1*x98
    x100 = x31*x99
    x101 = x100*x47 + x95
    x102 = -x34*(x*x40 + x101*x52) + x34*(x*x6 + x23*(x100*x27 + x95))
    x103 = x53*x66
    x104 = x101*x70
    x105 = 0.5*x104
    x106 = -x*x64 - x103*x35*x98 - x105*x72
    x107 = e*x96
    x108 = x42*x50
    x109 = x108*x63*x66
    x110 = x51*x83*x88*x91
    x111 = 2*r_cut*x102*x58 + 2*r_cut*x106*x32*x33*x56 - x102*x57 - x106*x77 - x94*(2.0*x*x107*x108*x31*x70/pow(x43, 2) - x*x90 + x*x92 + x101*x110 - x103*x99 + x104*x109 - x104*x86 - x105*x84 + 6*x53*x80*x97)
    x112 = x15*x19
    x113 = -4*x107 - x24*x9/x11
    x114 = x15*x48
    x115 = x113 + x114*x19 + x15*x46
    x116 = -x34*(x115*x52 - x41) + x34*(x23*(x112*x29 + x113 + x15*x26) - x7)
    x117 = 2.0*r_cut*x1*x14*x47*x8 - x112*x67
    x118 = x115*x70
    x119 = 0.5*x118
    x120 = x117*x52 - x119*x72 - x59 + x65
    return [2*r_cut*x32*x33*x56*x75 + 2*r_cut*x54*x58 - x54*x57 - x75*x77 - x94*(2.0*e*x10*x30*x42*x49*x51*x82*x88 + 0.5*x1*x10*x30*x42*x49*x50*x66*x70 + x10*x30*x52*(-x17*x78 + x17*x81 - x48 + x79) - x31*x68*x87 - 0.5*x84*x85 - x85*x86 - x93), x111, x111, 2*r_cut*x116*x58 + 2*r_cut*x120*x32*x33*x56 - x116*x57 - x120*x77 - x94*(x109*x118 + x110*x115 - x117*x87 - x118*x86 - x119*x84 + x52*(-x112*x78 + x112*x81 + x114 - x15*x79) + x93)]


@tensorflow.function
def hessian_3(x, y, e, r_core, r_cut):
    x0 = -r_core + r_cut
    x1 = pow(x0, -3)
    x2 = -y
    x3 = sqrt(e)
    x4 = r_core*x3
    x5 = pow(x, 2)
    x6 = 0.5*y
    x7 = -x6
    x8 = pow(x5 + 4.0*pow(x4 + x7, 2), -1)
    x9 = x8*(x2 + 2.0*x4)
    x10 = e + 1
    x11 = pow(x10, 2)
    x12 = 1 - e
    x13 = pow(x12, 2)
    x14 = x13/x11
    x15 = x14*x5
    x16 = pow(x12, -1)
    x17 = x10*x16
    x18 = -x17*x6
    x19 = pow(e + 1.0, -2)
    x20 = x19*x5
    x21 = pow(y, 2)
    x22 = pow(1.0 - e, -2)
    x23 = x21*x22
    x24 = x20 + x23
    x25 = sqrt(pow(r_core, 2) + x24)
    x26 = x25*x3
    x27 = pow(x15 + 4.0*pow(x18 + x26, 2), -1)
    x28 = 1.0*y
    x29 = x17*x28
    x30 = 2.0*x26
    x31 = pow(x25, -1)
    x32 = 2.0*x3
    x33 = x31*x32
    x34 = pow(x10, -1)
    x35 = x12*x34
    x36 = pow(x3, -1)
    x37 = 0.5 - 0.5*pow(e, 2)
    x38 = x36*x37
    x39 = r_cut*x3
    x40 = 2.0*x39
    x41 = x2 + x40
    x42 = x39 + x7
    x43 = pow(x42, 2)
    x44 = pow(4.0*x43 + x5, -1)
    x45 = x41*x44
    x46 = pow(r_cut, 2)
    x47 = x24 + x46
    x48 = sqrt(x47)
    x49 = x3*x48
    x50 = 2.0*x49
    x51 = pow(x48, -1)
    x52 = x32*x51
    x53 = x20*x52 + x29 - x50
    x54 = x18 + x49
    x55 = pow(x54, 2)
    x56 = pow(x15 + 4.0*x55, -1)
    x57 = x35*x56
    x58 = -x38*(x45 + x53*x57) + x38*(x27*x35*(x20*x33 + x29 - x30) + x9)
    x59 = 6*r_cut/pow(x0, 4)
    x60 = pow(x0, -2)
    x61 = x32*x44
    x62 = x41*x42
    x63 = 0.25*x5
    x64 = x43 + x63
    x65 = pow(x64, -2)
    x66 = 0.5*x3
    x67 = x65*x66
    x68 = x62*x67
    x69 = pow(x47, -3/2)
    x70 = x40*x69
    x71 = -x20*x70 - x40*x51
    x72 = x51*x54
    x73 = x39*x72
    x74 = 0.5*x73
    x75 = x14*x63 + x55
    x76 = pow(x75, -2)
    x77 = x35*x53
    x78 = x76*x77
    x79 = x12*x34*x56*x71 + x61 - x68 - x74*x78
    x80 = 6*r_cut*x1
    x81 = x32*x69
    x82 = x46*x81
    x83 = pow(x47, -5/2)
    x84 = 6.0*x83
    x85 = x3*x46
    x86 = x84*x85
    x87 = -x20*x81 + x20*x86 - x52 + x82
    x88 = pow(x47, -1)
    x89 = x46*x88
    x90 = 0.5*x89
    x91 = x66*x72
    x92 = 1.0*x73
    x93 = x35*x71
    x94 = pow(x75, -3)
    x95 = e*x65
    x96 = 0.5*x95
    x97 = pow(x64, -3)
    x98 = x43*x97
    x99 = 2.0*e
    x100 = x98*x99
    x101 = -x100*x41 + x41*x96 + 2.0*x42*x95
    x102 = 2.0*e*x12*x34*x46*x53*x55*x88*x94 - e*x78*x90 - x101 + 0.5*x12*x3*x34*x46*x53*x54*x69*x76 + x12*x34*x56*x87 - x76*x92*x93 - x78*x91
    x103 = x38/x0
    x104 = 3*x103
    x105 = pow(e, 3/2)
    x106 = 3.0*x105*x65
    x107 = 12.0*x105
    x108 = x107*x98
    x109 = 6.0*x105*x62*x97
    x110 = x107*pow(x42, 3)/pow(x64, 4)
    x111 = x110*x41
    x112 = e*x89
    x113 = 1.5*x76
    x114 = x113*x93
    x115 = r_cut*x88
    x116 = x113*x77
    x117 = pow(r_cut, 3)
    x118 = pow(x47, -2)
    x119 = x3*x72
    x120 = x117*x3
    x121 = x120/pow(x47, 7/2)
    x122 = 30.0*x121
    x123 = x120*x83
    x124 = x117*x118
    x125 = 6.0*e
    x126 = x55*x94
    x127 = x107*x117*pow(x54, 3)*x69/pow(x75, 4)
    x128 = x113*x73
    x129 = r_cut*x103
    x130 = -x
    x131 = x22*y
    x132 = x*x131
    x133 = 2*x132
    x134 = x133*x3
    x135 = x134*x35
    x136 = x130 + x135*x51
    x137 = -x38*(x*x44 + x136*x56) + x38*(x*x8 + x27*(x130 + x135*x31))
    x138 = x39*x69
    x139 = x136*x76
    x140 = -x*x42*x67 - x133*x138*x57 - x139*x74
    x141 = x38*x80
    x142 = x132*x57
    x143 = e*x139
    x144 = x35*x76
    x145 = e*x131
    x146 = x*x145*x54
    x147 = x139*x54
    x148 = x46*x66*x69
    x149 = x136*x55*x94
    x150 = x89*x99
    x151 = x*x100 - x*x96 + 2.0*x118*x144*x146*x46 - x134*x57*x69 - x139*x91 + 6*x142*x83*x85 - x143*x90 + x147*x148 + x149*x150
    x152 = 3*r_cut*x151*x36*x37*x60 + 6*x1*x137 - x104*x151 - x129*(6.0*e*r_cut*x*x118*x12*x22*x34*x54*x76*y + 6.0*e*r_cut*x136*x55*x88*x94 + 1.5*e*x117*x118*x136*x76 + 18*r_cut*x*x12*x22*x3*x34*x56*x83*y + 1.5*r_cut*x136*x3*x54*x69*x76 + 3.0*x*x105*x117*x12*x22*x34*x76*x83*y + 6.0*x*x105*x42*x97 - x*x110 + 6.0*x105*x117*x136*x54*x69*x94 - x107*x117*x126*x132*x35*x83 - 1.5*x115*x143 - 12.0*x117*x144*x146/pow(x47, 3) - 30*x121*x142 - 1.5*x123*x147 - x124*x125*x149 - x127*x136) - x137*x59 - x140*x141 + 6*x140*x36*x37*x60
    x153 = x17*x23
    x154 = -x11*x28/x13 - 4*x145
    x155 = x17*x52
    x156 = x154 + x155*x23 + x17*x50
    x157 = -x38*(x156*x56 - x45) + x38*(x27*(x153*x33 + x154 + x17*x30) - x9)
    x158 = 2.0*r_cut*x10*x16*x3*x51 - x153*x70
    x159 = x156*x76
    x160 = x158*x56 - x159*x74 - x61 + x68
    x161 = -x153*x81 + x153*x86 + x155 - x17*x82
    x162 = e*x159
    x163 = x159*x54
    x164 = x126*x156
    x165 = x158*x76
    x166 = x101 + x148*x163 + x150*x164 - x159*x91 + x161*x56 - x162*x90 - x165*x92
    x167 = 1.5*x165
    return [3*r_cut*x102*x36*x37*x60 + 6*x1*x58 - x102*x104 - x129*(6.0*e*r_cut*x12*x34*x53*x55*x88*x94 - e*x115*x116 + 1.5*e*x117*x118*x12*x34*x53*x76 + 6.0*e*x12*x34*x46*x55*x71*x88*x94 + 1.5*r_cut*x12*x3*x34*x53*x54*x69*x76 + 6.0*x105*x117*x12*x34*x53*x54*x69*x94 - x106 + x108 + x109 - x111 - x112*x114 - x114*x119 - x116*x123*x54 + 1.5*x12*x3*x34*x46*x54*x69*x71*x76 + x12*x34*x56*(18.0*r_cut*x19*x3*x5*x83 + 6.0*r_cut*x3*x69 - x120*x84 - x122*x20) - x124*x125*x126*x77 - x127*x77 - x128*x35*x87) + 6*x36*x37*x60*x79 - x38*x79*x80 - x58*x59, x152, x152, 3*r_cut*x166*x36*x37*x60 + 6*x1*x157 - x104*x166 - x129*(6.0*e*r_cut*x156*x55*x88*x94 + 1.5*e*x117*x118*x156*x76 + 6.0*e*x158*x46*x55*x88*x94 + 1.5*r_cut*x156*x3*x54*x69*x76 + 6.0*x105*x117*x156*x54*x69*x94 + x106 - x108 - x109 + x111 - x112*x167 - 1.5*x115*x162 - x119*x167 - 1.5*x123*x163 - x124*x125*x164 - x127*x156 - x128*x161 + 1.5*x158*x3*x46*x54*x69*x76 + x56*(18.0*r_cut*x10*x16*x21*x22*x3*x83 + 6.0*x10*x117*x16*x3*x83 - x122*x153 - 6.0*x138*x17)) - x141*x160 - x157*x59 + 6*x160*x36*x37*x60]


@tensorflow.function
def hessian_4(x, y, e, r_core, r_cut):
    x0 = -y
    x1 = sqrt(e)
    x2 = r_core*x1
    x3 = pow(x, 2)
    x4 = 0.5*y
    x5 = -x4
    x6 = pow(x3 + 4.0*pow(x2 + x5, 2), -1)
    x7 = x6*(x0 + 2.0*x2)
    x8 = e + 1
    x9 = pow(x8, 2)
    x10 = 1 - e
    x11 = pow(x10, 2)
    x12 = x11/x9
    x13 = x12*x3
    x14 = pow(x10, -1)
    x15 = x14*x8
    x16 = -x15*x4
    x17 = pow(e + 1.0, -2)
    x18 = x17*x3
    x19 = pow(y, 2)
    x20 = pow(1.0 - e, -2)
    x21 = x19*x20
    x22 = x18 + x21
    x23 = sqrt(pow(r_core, 2) + x22)
    x24 = x1*x23
    x25 = pow(x13 + 4.0*pow(x16 + x24, 2), -1)
    x26 = 1.0*y
    x27 = x15*x26
    x28 = 2.0*x24
    x29 = pow(x23, -1)
    x30 = 2.0*x1
    x31 = x29*x30
    x32 = pow(x8, -1)
    x33 = x10*x32
    x34 = pow(x1, -1)
    x35 = pow(e, 2)
    x36 = 0.5 - 0.5*x35
    x37 = x34*x36
    x38 = r_cut*x1
    x39 = 2.0*x38
    x40 = x0 + x39
    x41 = x38 + x5
    x42 = pow(x41, 2)
    x43 = pow(x3 + 4.0*x42, -1)
    x44 = x40*x43
    x45 = pow(r_cut, 2)
    x46 = x22 + x45
    x47 = sqrt(x46)
    x48 = x1*x47
    x49 = 2.0*x48
    x50 = pow(x47, -1)
    x51 = x30*x50
    x52 = x18*x51 + x27 - x49
    x53 = x16 + x48
    x54 = pow(x53, 2)
    x55 = pow(x13 + 4.0*x54, -1)
    x56 = x33*x55
    x57 = -x37*(x44 + x52*x56) + x37*(x25*x33*(x18*x31 + x27 - x28) + x7)
    x58 = -r_core + r_cut
    x59 = pow(x58, -4)
    x60 = 24*x59
    x61 = pow(x58, -5)
    x62 = x30*x43
    x63 = 0.25*x3
    x64 = x42 + x63
    x65 = pow(x64, -2)
    x66 = 0.5*x1
    x67 = x41*x65*x66
    x68 = x40*x67
    x69 = x39*x50
    x70 = pow(x46, -3/2)
    x71 = x39*x70
    x72 = -x18*x71 - x69
    x73 = x50*x53
    x74 = x38*x73
    x75 = 0.5*x74
    x76 = x12*x63 + x54
    x77 = pow(x76, -2)
    x78 = x33*x52
    x79 = x77*x78
    x80 = x10*x32*x55*x72 + x62 - x68 - x75*x79
    x81 = x37/pow(x58, 3)
    x82 = 24*x81
    x83 = pow(x58, -2)
    x84 = x30*x70
    x85 = x45*x84
    x86 = pow(x46, -5/2)
    x87 = x1*x86
    x88 = x45*x87
    x89 = 6.0*x88
    x90 = -x18*x84 + x18*x89 - x51 + x85
    x91 = pow(x46, -1)
    x92 = e*x91
    x93 = 0.5*x92
    x94 = x45*x77
    x95 = x78*x94
    x96 = x66*x73
    x97 = 1.0*x74
    x98 = x33*x77
    x99 = x72*x98
    x100 = pow(x76, -3)
    x101 = e*x65
    x102 = 0.5*x101
    x103 = x40*x42
    x104 = pow(x64, -3)
    x105 = 2.0*e
    x106 = x104*x105
    x107 = 2.0*x101*x41 + x102*x40 - x103*x106
    x108 = 2.0*e*x10*x100*x32*x45*x52*x54*x91 + 0.5*x1*x10*x32*x45*x52*x53*x70*x77 + x10*x32*x55*x90 - x107 - x79*x96 - x93*x95 - x97*x99
    x109 = r_cut*x81
    x110 = pow(e, 3/2)
    x111 = 3.0*x110*x65
    x112 = x104*x110
    x113 = 12.0*x112*x42
    x114 = 6.0*x40
    x115 = x112*x114*x41
    x116 = pow(x64, -4)
    x117 = x116*pow(x41, 3)
    x118 = 12.0*x110
    x119 = x117*x118
    x120 = x119*x40
    x121 = x45*x92
    x122 = 1.5*x77
    x123 = x122*x33
    x124 = x123*x72
    x125 = r_cut*x92
    x126 = x123*x52
    x127 = pow(r_cut, 3)
    x128 = pow(x46, -2)
    x129 = x1*x73
    x130 = 6.0*x127
    x131 = x130*x87
    x132 = x1*x18
    x133 = pow(x46, -7/2)
    x134 = 30.0*x133
    x135 = x127*x134
    x136 = 18.0*r_cut*x1*x17*x3*x86 + 6.0*r_cut*x1*x70 - x131 - x132*x135
    x137 = x53*x87
    x138 = x127*x137
    x139 = e*x128
    x140 = x130*x139
    x141 = x100*x54
    x142 = x141*x78
    x143 = pow(x76, -4)
    x144 = x143*pow(x53, 3)
    x145 = x144*x70
    x146 = x118*x145
    x147 = 6.0*e*r_cut*x10*x100*x32*x52*x54*x91 + 6.0*e*x10*x100*x32*x45*x54*x72*x91 + 1.5*e*x10*x127*x128*x32*x52*x77 + 1.5*r_cut*x1*x10*x32*x52*x53*x70*x77 + 1.5*x1*x10*x32*x45*x53*x70*x72*x77 + 6.0*x10*x100*x110*x127*x32*x52*x53*x70 + x10*x136*x32*x55 - x111 + x113 + x115 - x120 - x121*x124 - x123*x74*x90 - x124*x129 - x125*x126 - x126*x138 - x127*x146*x78 - x140*x142
    x148 = x37/x58
    x149 = 4*x148
    x150 = x104*x35
    x151 = 48.0*x150*x41
    x152 = x114*x150
    x153 = 96.0*x35
    x154 = x117*x153
    x155 = x153*pow(x41, 4)/pow(x64, 5)
    x156 = x155*x40
    x157 = 72.0*x35
    x158 = x116*x157
    x159 = x103*x158
    x160 = 6.0*x125
    x161 = 9.0*x139
    x162 = pow(r_cut, 4)
    x163 = pow(x46, -3)
    x164 = x162*x163
    x165 = e*x164
    x166 = 7.5*x79
    x167 = 6.0*x100
    x168 = x167*x35
    x169 = x128*x162
    x170 = x169*x78
    x171 = 6.0*x70
    x172 = x171*x38
    x173 = x53*x77
    x174 = x33*x72
    x175 = x173*x174
    x176 = 24.0*x174
    x177 = x141*x176
    x178 = x127*x139
    x179 = x53*x70
    x180 = x100*x110
    x181 = x179*x180
    x182 = x127*x181
    x183 = 48.0*x110*x127*x145
    x184 = x1*x179
    x185 = x167*x92
    x186 = x54*x78
    x187 = 9.0*x53*x88
    x188 = x1*x162
    x189 = x133*x188*x53
    x190 = 36.0*x139
    x191 = x45*x54
    x192 = 30.0*x165
    x193 = 36.0*x181
    x194 = x45*x78
    x195 = 36.0*x180*x53
    x196 = x162*x86
    x197 = x196*x78
    x198 = 72.0*x110
    x199 = x145*x198
    x200 = x144*x198
    x201 = x143*x157
    x202 = x169*x201
    x203 = x153*pow(x53, 4)/pow(x76, 5)
    x204 = 3.0*x90
    x205 = x204*x98
    x206 = x1*x171
    x207 = 36.0*x88
    x208 = x134*x188
    x209 = x133*x45
    x210 = 180.0*x209
    x211 = x188/pow(x46, 9/2)
    x212 = 210.0*x211
    x213 = x173*x69
    x214 = x184*x94
    x215 = 12.0*x121*x141
    x216 = r_cut*x148
    x217 = -x
    x218 = x20*y
    x219 = x*x218
    x220 = 2*x219
    x221 = x1*x220
    x222 = x221*x33
    x223 = x217 + x222*x50
    x224 = -x37*(x*x43 + x223*x55) + x37*(x*x6 + x25*(x217 + x222*x29))
    x225 = x56*x70
    x226 = x223*x77
    x227 = -x*x67 - x220*x225*x38 - x226*x75
    x228 = x*x42
    x229 = x219*x56
    x230 = x226*x45
    x231 = e*x218
    x232 = x231*x33
    x233 = x*x232*x53*x94
    x234 = x179*x66
    x235 = x223*x54
    x236 = x100*x235
    x237 = x236*x45
    x238 = x105*x91
    x239 = -x*x102 + x106*x228 + 2.0*x128*x233 - x221*x225 - x226*x96 + 6*x229*x88 + x230*x234 - x230*x93 + x237*x238
    x240 = 12*x109
    x241 = x1*x229
    x242 = 1.5*x226
    x243 = x242*x92
    x244 = x173*x232
    x245 = x*x244
    x246 = x219*x86
    x247 = 6.0*e*r_cut*x*x10*x128*x20*x32*x53*x77*y + 6.0*e*r_cut*x100*x223*x54*x91 + 1.5*e*x127*x128*x223*x77 + 18*r_cut*x*x1*x10*x20*x32*x55*x86*y + 1.5*r_cut*x1*x223*x53*x70*x77 - r_cut*x243 + 3.0*x*x10*x110*x127*x20*x32*x77*x86*y + 6.0*x*x104*x110*x41 - x*x119 + 6.0*x100*x110*x127*x223*x53*x70 - x118*x127*x141*x246*x33 - 30*x127*x133*x241 - x127*x137*x242 - x127*x146*x223 - 12.0*x127*x163*x245 - x140*x236
    x248 = 6.0*x
    x249 = x246*x33
    x250 = x162*x219
    x251 = 7.5*x226
    x252 = x162*x223
    x253 = x128*x252
    x254 = x164*x219*x33
    x255 = x223*x45
    x256 = x252*x86
    x257 = 24*r_cut*x224*x61 + 24*r_cut*x227*x34*x36*x59 + 4*r_cut*x247*x34*x36*x83 - x149*x247 - x216*(x*x155 - 48.0*x100*x254*x35*x53 - x110*x134*x250*x98 + 18.0*x110*x249*x94 + x128*x244*x248 + 120.0*x133*x180*x250*x33*x54 + x144*x153*x254 + x150*x248 - x158*x228 + x161*x230 + 90.0*x162*x245/pow(x46, 4) - 72.0*x163*x233 - x165*x251 + x168*x253 - 72.0*x180*x191*x249 + x184*x242 + x185*x235 - x187*x226 + x189*x251 - x190*x237 + x192*x236 + x193*x255 - x195*x256 - x199*x255 + x200*x256 - x202*x235 + x203*x253 - 180*x209*x241 + 210*x211*x229 + 18*x229*x87 - x243) - x224*x60 - x227*x82 - x239*x240 + 12*x239*x34*x36*x83
    x258 = x15*x21
    x259 = -4*x231 - x26*x9/x11
    x260 = x15*x51
    x261 = x15*x49 + x21*x260 + x259
    x262 = x37*(x25*(x15*x28 + x258*x31 + x259) - x7) - x37*(x261*x55 - x44)
    x263 = 2.0*r_cut*x1*x14*x50*x8 - x258*x71
    x264 = x261*x77
    x265 = x263*x55 - x264*x75 - x62 + x68
    x266 = -x15*x85 - x258*x84 + x258*x89 + x260
    x267 = x261*x92
    x268 = x261*x94
    x269 = x100*x261
    x270 = x191*x269
    x271 = x263*x77
    x272 = x107 + x234*x268 + x238*x270 - x264*x96 + x266*x55 - 0.5*x267*x94 - x271*x97
    x273 = x122*x263
    x274 = x122*x267
    x275 = x1*x258
    x276 = 18.0*r_cut*x1*x14*x19*x20*x8*x86 + 6.0*x1*x127*x14*x8*x86 - x135*x275 - x15*x172
    x277 = x122*x261
    x278 = x269*x54
    x279 = 6.0*e*r_cut*x100*x261*x54*x91 + 6.0*e*x100*x263*x45*x54*x91 + 1.5*e*x127*x128*x261*x77 + 1.5*r_cut*x1*x261*x53*x70*x77 - r_cut*x274 + 1.5*x1*x263*x45*x53*x70*x77 + 6.0*x100*x110*x127*x261*x53*x70 + x111 - x113 - x115 + x120 - x121*x273 - x122*x266*x74 - x127*x146*x261 - x129*x273 - x138*x277 - x140*x278 + x276*x55
    x280 = 7.5*x264
    x281 = x169*x261
    x282 = x271*x53
    x283 = 24.0*x263
    x284 = x141*x283
    x285 = x261*x45
    x286 = x196*x261
    x287 = 3.0*x266
    x288 = x287*x77
    return [4*r_cut*x147*x34*x36*x83 + 24*r_cut*x34*x36*x59*x80 + 24*r_cut*x57*x61 - 12*x108*x109 + 12*x108*x34*x36*x83 - x147*x149 - x216*(-x100*x190*x191*x78 - x121*x205 + x125*x177 + x126*x184 - x126*x92 - x129*x205 - x131*x175 - x136*x213*x33 + x140*x99 + x142*x192 + x151 + x152 - x154 + x156 - x159 - x160*x99 + x161*x95 - x165*x166 + x166*x189 + x168*x170 + x170*x203 + x172*x175 - x174*x183 + x176*x182 - x177*x178 + x185*x186 - x186*x202 - x187*x79 + x193*x194 - x194*x199 - x195*x197 + x197*x200 + x204*x214*x33 + x215*x33*x90 + x56*(-x132*x210 + x18*x212 + 18.0*x18*x87 + x206 - x207 + x208)) - x57*x60 - x80*x82, x257, x257, 24*r_cut*x262*x61 + 24*r_cut*x265*x34*x36*x59 + 4*r_cut*x279*x34*x36*x83 - x149*x279 - x216*(-x121*x288 + x125*x284 - x129*x288 - x131*x282 + x140*x271 - x151 - x152 + x154 - x156 + x159 - x160*x271 + x161*x268 - x165*x280 + x167*x267*x54 + x168*x281 + x172*x282 - x178*x284 + x182*x283 - x183*x263 + x184*x277 - x187*x264 + x189*x280 - x190*x270 + x192*x278 + x193*x285 - x195*x286 - x199*x285 + x200*x286 - x201*x281*x54 + x203*x281 - x213*x276 + x214*x287 + x215*x266 - x274 + x55*(-x15*x206 + x15*x207 - x15*x208 - x210*x275 + x212*x258 + 18.0*x258*x87)) - x240*x272 - x262*x60 - x265*x82 + 12*x272*x34*x36*x83]


@tensorflow.function
def hessian_5(x, y, e, r_core, r_cut):
    x0 = -r_core + r_cut
    x1 = pow(x0, -5)
    x2 = -y
    x3 = sqrt(e)
    x4 = r_core*x3
    x5 = pow(x, 2)
    x6 = 0.5*y
    x7 = -x6
    x8 = pow(x5 + 4.0*pow(x4 + x7, 2), -1)
    x9 = x8*(x2 + 2.0*x4)
    x10 = e + 1
    x11 = pow(x10, 2)
    x12 = 1 - e
    x13 = pow(x12, 2)
    x14 = x13/x11
    x15 = x14*x5
    x16 = pow(x12, -1)
    x17 = x10*x16
    x18 = -x17*x6
    x19 = pow(e + 1.0, -2)
    x20 = x19*x5
    x21 = pow(y, 2)
    x22 = pow(1.0 - e, -2)
    x23 = x21*x22
    x24 = x20 + x23
    x25 = sqrt(pow(r_core, 2) + x24)
    x26 = x25*x3
    x27 = pow(x15 + 4.0*pow(x18 + x26, 2), -1)
    x28 = 1.0*y
    x29 = x17*x28
    x30 = 2.0*x26
    x31 = pow(x25, -1)
    x32 = 2.0*x3
    x33 = x31*x32
    x34 = pow(x10, -1)
    x35 = x12*x34
    x36 = pow(x3, -1)
    x37 = pow(e, 2)
    x38 = 0.5 - 0.5*x37
    x39 = x36*x38
    x40 = r_cut*x3
    x41 = 2.0*x40
    x42 = x2 + x41
    x43 = x40 + x7
    x44 = pow(x43, 2)
    x45 = pow(4.0*x44 + x5, -1)
    x46 = x42*x45
    x47 = pow(r_cut, 2)
    x48 = x24 + x47
    x49 = sqrt(x48)
    x50 = x3*x49
    x51 = 2.0*x50
    x52 = pow(x49, -1)
    x53 = x32*x52
    x54 = x20*x53 + x29 - x51
    x55 = x18 + x50
    x56 = pow(x55, 2)
    x57 = pow(x15 + 4.0*x56, -1)
    x58 = x35*x57
    x59 = -x39*(x46 + x54*x58) + x39*(x27*x35*(x20*x33 + x29 - x30) + x9)
    x60 = 120*r_cut/pow(x0, 6)
    x61 = pow(x0, -4)
    x62 = x32*x45
    x63 = x42*x43
    x64 = 0.25*x5
    x65 = x44 + x64
    x66 = pow(x65, -2)
    x67 = 0.5*x3
    x68 = x66*x67
    x69 = x63*x68
    x70 = x41*x52
    x71 = pow(x48, -3/2)
    x72 = x41*x71
    x73 = -x20*x72 - x70
    x74 = x52*x55
    x75 = x40*x74
    x76 = 0.5*x75
    x77 = x14*x64 + x56
    x78 = pow(x77, -2)
    x79 = x35*x78
    x80 = x54*x79
    x81 = x12*x34*x57*x73 + x62 - x69 - x76*x80
    x82 = 120*r_cut*x1
    x83 = x32*x71
    x84 = x47*x83
    x85 = pow(x48, -5/2)
    x86 = x3*x85
    x87 = 6.0*x86
    x88 = x20*x47
    x89 = -x20*x83 - x53 + x84 + x87*x88
    x90 = 0.5*x47
    x91 = pow(x48, -1)
    x92 = e*x91
    x93 = x78*x92
    x94 = x35*x93
    x95 = x54*x94
    x96 = x67*x74
    x97 = 1.0*x75
    x98 = x73*x79
    x99 = pow(x77, -3)
    x100 = e*x66
    x101 = 0.5*x100
    x102 = pow(x65, -3)
    x103 = x102*x44
    x104 = 2.0*e
    x105 = x103*x104
    x106 = 2.0*x100*x43 + x101*x42 - x105*x42
    x107 = 2.0*e*x12*x34*x47*x54*x56*x91*x99 - x106 + 0.5*x12*x3*x34*x47*x54*x55*x71*x78 + x12*x34*x57*x89 - x80*x96 - x90*x95 - x97*x98
    x108 = x39/pow(x0, 3)
    x109 = 60*x108
    x110 = pow(x0, -2)
    x111 = pow(e, 3/2)
    x112 = 3.0*x111*x66
    x113 = 12.0*x111
    x114 = x103*x113
    x115 = 6.0*x102*x111*x63
    x116 = pow(x65, -4)
    x117 = pow(x43, 3)
    x118 = x116*x117
    x119 = x113*x118
    x120 = x119*x42
    x121 = x47*x93
    x122 = x121*x35
    x123 = r_cut*x93
    x124 = x35*x54
    x125 = pow(r_cut, 3)
    x126 = pow(x48, -2)
    x127 = x3*x74
    x128 = 1.5*x127
    x129 = x125*x87
    x130 = pow(x48, -7/2)
    x131 = x130*x3
    x132 = x125*x131
    x133 = 30.0*x132
    x134 = 18.0*r_cut*x19*x3*x5*x85 + 6.0*r_cut*x3*x71 - x129 - x133*x20
    x135 = x55*x86
    x136 = x125*x135
    x137 = 1.5*x80
    x138 = e*x126
    x139 = x125*x138
    x140 = 6.0*x139
    x141 = x124*x99
    x142 = x141*x56
    x143 = x113*x125
    x144 = pow(x77, -4)
    x145 = pow(x55, 3)
    x146 = x144*x145
    x147 = x146*x71
    x148 = x124*x147
    x149 = x75*x78
    x150 = 1.5*x149
    x151 = x35*x89
    x152 = 6.0*e*r_cut*x12*x34*x54*x56*x91*x99 + 1.5*e*x12*x125*x126*x34*x54*x78 + 6.0*e*x12*x34*x47*x56*x73*x91*x99 + 1.5*r_cut*x12*x3*x34*x54*x55*x71*x78 + 6.0*x111*x12*x125*x34*x54*x55*x71*x99 - x112 + x114 + x115 + x12*x134*x34*x57 + 1.5*x12*x3*x34*x47*x55*x71*x73*x78 - x120 - 1.5*x122*x73 - 1.5*x123*x124 - x128*x98 - x136*x137 - x140*x142 - x143*x148 - x150*x151
    x153 = r_cut*x108
    x154 = x102*x37
    x155 = 48.0*x154*x43
    x156 = 6.0*x154
    x157 = x156*x42
    x158 = 96.0*x37
    x159 = x118*x158
    x160 = pow(x65, -5)
    x161 = x160*pow(x43, 4)
    x162 = x158*x161
    x163 = x162*x42
    x164 = x116*x44
    x165 = 72.0*x37
    x166 = x164*x165
    x167 = x166*x42
    x168 = 6.0*r_cut
    x169 = x73*x94
    x170 = x138*x47
    x171 = 9.0*x80
    x172 = pow(r_cut, 4)
    x173 = 7.5*x172
    x174 = pow(x48, -3)
    x175 = e*x174
    x176 = x175*x80
    x177 = x126*x37
    x178 = x172*x177
    x179 = x178*x99
    x180 = 6.0*x124
    x181 = 6.0*x71
    x182 = x181*x40
    x183 = x55*x78
    x184 = x35*x73
    x185 = x183*x184
    x186 = 24.0*r_cut
    x187 = x56*x92
    x188 = x187*x99
    x189 = x139*x56
    x190 = x184*x99
    x191 = 24.0*x190
    x192 = x55*x71
    x193 = x111*x192
    x194 = x125*x193
    x195 = x111*x125
    x196 = 48.0*x195
    x197 = x147*x184
    x198 = x192*x3
    x199 = x135*x47
    x200 = x131*x173
    x201 = x124*x183
    x202 = 36.0*x47
    x203 = x138*x202
    x204 = 30.0*x172
    x205 = x111*x85
    x206 = 36.0*x172
    x207 = x205*x206
    x208 = x141*x55
    x209 = x111*x47
    x210 = 72.0*x209
    x211 = 72.0*x205
    x212 = x146*x211
    x213 = x124*x172
    x214 = x144*x56
    x215 = x165*x214
    x216 = x126*x213
    x217 = pow(x77, -5)
    x218 = pow(x55, 4)
    x219 = x217*x218
    x220 = x158*x219
    x221 = 3.0*x89
    x222 = x127*x79
    x223 = x181*x3
    x224 = x202*x86
    x225 = x131*x204
    x226 = 180.0*x131
    x227 = pow(x48, -9/2)
    x228 = x227*x3
    x229 = 210.0*x172
    x230 = x20*x228*x229 + 18.0*x20*x86 + x223 - x224 + x225 - x226*x88
    x231 = x183*x70
    x232 = x198*x47
    x233 = 12.0*x188*x47
    x234 = -x122*x221 - x129*x185 - x134*x231*x35 + x137*x198 + x140*x98 + x141*x175*x204*x56 + x141*x193*x202 - x142*x203 - x148*x210 + x151*x233 + x155 + x157 - x159 + x163 - x167 - x168*x169 + x170*x171 - x171*x199 - x173*x176 + x179*x180 + x180*x188 + x182*x185 + x184*x186*x188 - x189*x191 + x191*x194 - x196*x197 + x200*x201 - x207*x208 + x212*x213 - x215*x216 + x216*x220 - x221*x222 + x221*x232*x79 + x230*x58 - 1.5*x95
    x235 = x39/x0
    x236 = 5*x235
    x237 = pow(e, 5/2)
    x238 = 60.0*x237
    x239 = x102*x238
    x240 = 960.0*x237
    x241 = x161*x240
    x242 = 720.0*x164*x237
    x243 = 180.0*x237
    x244 = x116*x243
    x245 = x244*x63
    x246 = x240*x42
    x247 = pow(x43, 5)/pow(x65, 6)
    x248 = x246*x247
    x249 = x117*x160*x246
    x250 = 37.5*x172
    x251 = 75.0*x125
    x252 = pow(r_cut, 5)
    x253 = pow(x48, -4)
    x254 = x174*x37
    x255 = x252*x254
    x256 = x170*x56
    x257 = 180.0*x190
    x258 = x172*x205*x55
    x259 = 360.0*x209
    x260 = 360.0*x214
    x261 = x40*x85
    x262 = x261*x55
    x263 = 90.0*r_cut*x138
    x264 = 52.5*x228*x252
    x265 = 210.0*x252
    x266 = e*x253*x265
    x267 = 180.0*r_cut
    x268 = x111*x267
    x269 = 360.0*x125*x205
    x270 = x144*x243*x55
    x271 = x252*x85
    x272 = x124*x271
    x273 = x130*x252
    x274 = 540.0*x111*x273
    x275 = x125*x177
    x276 = 720.0*x214
    x277 = 960.0*x219
    x278 = pow(x55, 5)/pow(x77, 6)
    x279 = 5.0*x134
    x280 = 120.0*x147*x195
    x281 = x130*x40
    x282 = 450.0*x281
    x283 = x252*x3/pow(x48, 11/2)
    x284 = 1890.0*x283
    x285 = 2.5*x149
    x286 = r_cut*x235
    x287 = -x
    x288 = x22*y
    x289 = x*x288
    x290 = 2*x289
    x291 = x290*x3
    x292 = x291*x35
    x293 = x287 + x292*x52
    x294 = -x39*(x*x45 + x293*x57) + x39*(x*x8 + x27*(x287 + x292*x31))
    x295 = x*x43
    x296 = x58*x71
    x297 = x293*x78
    x298 = -x290*x296*x40 - x295*x68 - x297*x76
    x299 = x39*x82
    x300 = x289*x58
    x301 = x300*x86
    x302 = x297*x47
    x303 = e*x288
    x304 = x*x183*x303*x35
    x305 = x126*x304
    x306 = x192*x67
    x307 = x293*x99
    x308 = x307*x56
    x309 = x104*x47*x91
    x310 = -x*x101 + x*x105 - x291*x296 - x297*x96 + 6*x301*x47 + x302*x306 - 0.5*x302*x92 + 2.0*x305*x47 + x308*x309
    x311 = 1.5*x297
    x312 = x311*x92
    x313 = x174*x304
    x314 = x289*x35
    x315 = x314*x99
    x316 = x315*x56
    x317 = 6.0*x307
    x318 = x146*x293
    x319 = x318*x71
    x320 = 6.0*e*r_cut*x*x12*x126*x22*x34*x55*x78*y + 6.0*e*r_cut*x293*x56*x91*x99 + 1.5*e*x125*x126*x293*x78 + 18*r_cut*x*x12*x22*x3*x34*x57*x85*y + 1.5*r_cut*x293*x3*x55*x71*x78 - r_cut*x312 + 6.0*x*x102*x111*x43 + 3.0*x*x111*x12*x125*x22*x34*x78*x85*y - x*x119 + 6.0*x111*x125*x293*x55*x71*x99 - 12.0*x125*x313 - 30*x132*x300 - x136*x311 - x143*x316*x85 - x143*x319 - x189*x317
    x321 = 20*x153
    x322 = x289*x79
    x323 = x111*x130*x322
    x324 = 9.0*x302
    x325 = e*x174*x297
    x326 = x172*x315
    x327 = x254*x55
    x328 = x146*x314
    x329 = x297*x55
    x330 = x111*x307
    x331 = x330*x55*x85
    x332 = x172*x293
    x333 = x126*x332
    x334 = x*x156 + x*x162 - x*x166 + 120.0*x111*x130*x326*x56 - 180*x131*x300*x47 - x135*x324 + x138*x324 + x158*x172*x174*x328 + 210*x172*x228*x300 + 90.0*x172*x253*x304 - x173*x325 + x175*x204*x308 + x178*x317 + x187*x317 + x192*x202*x330 + x198*x311 + x200*x329 - x203*x308 - x204*x323 + 18.0*x205*x322*x47 - x206*x331 - x210*x319 - x211*x316*x47 + x212*x332 - x215*x333 + x220*x333 + 18*x301 + 6.0*x305 - x312 - 72.0*x313*x47 - 48.0*x326*x327
    x335 = x*x240
    x336 = x271*x293
    x337 = 5*r_cut*x110*x334*x36*x38 + 60*r_cut*x310*x36*x38*x61 + 120*x1*x294 - x109*x310 + 20*x110*x320*x36*x38 - x236*x334 - x286*(22.5*e*r_cut*x126*x293*x78 + 900.0*e*x*x12*x125*x22*x253*x34*x55*x78*y + 300.0*e*x125*x174*x293*x56*x99 + 52.5*e*x252*x253*x293*x78 + 45.0*r_cut*x*x111*x12*x22*x34*x78*x85*y + 90.0*r_cut*x111*x293*x55*x71*x99 + 1200.0*x*x111*x12*x125*x130*x22*x34*x56*x99*y + 315.0*x*x111*x12*x22*x227*x252*x34*x78*y + 960.0*x*x117*x160*x237 + 960.0*x*x12*x125*x144*x145*x174*x22*x34*x37*y + 2100*x*x12*x125*x22*x227*x3*x34*x57*y + 720.0*x*x12*x130*x144*x22*x237*x252*x34*x56*y + 720.0*x*x12*x22*x252*x253*x34*x37*x55*x99*y + 720.0*x111*x125*x144*x145*x293*x85 + 270.0*x111*x130*x252*x293*x55*x99 - 1260.0*x111*x227*x252*x316 + 960.0*x125*x126*x217*x218*x293*x37 + 60.0*x125*x126*x293*x37*x99 + 75.0*x125*x130*x293*x3*x55*x78 - 480.0*x125*x315*x327 - 300.0*x125*x323 - 360.0*x125*x331 + 720.0*x144*x174*x252*x293*x37*x56 + 960.0*x145*x217*x237*x252*x293*x85 - x205*x267*x316 - x219*x273*x288*x335*x35 - x238*x273*x315 - x240*x278*x336 - x244*x295 - x247*x335 - x251*x325 - 1440.0*x252*x253*x328*x37 - 840.0*x252*x304/pow(x48, 5) - x255*x277*x293 - 60.0*x255*x307 - 22.5*x262*x297 - x263*x308 - x264*x329 - x266*x308 - x267*x313 - x268*x319 - x270*x336 - x274*x318 - x275*x276*x293 - 450*x281*x300 - 1890*x283*x300) - x294*x60 - x298*x299 + 120*x298*x36*x38*x61 - x320*x321
    x338 = x17*x23
    x339 = -x11*x28/x13 - 4*x303
    x340 = x17*x53
    x341 = x17*x51 + x23*x340 + x339
    x342 = x39*(x27*(x17*x30 + x33*x338 + x339) - x9) - x39*(x341*x57 - x46)
    x343 = 2.0*r_cut*x10*x16*x3*x52 - x338*x72
    x344 = x341*x78
    x345 = x343*x57 - x344*x76 - x62 + x69
    x346 = x338*x47
    x347 = -x17*x84 - x338*x83 + x340 + x346*x87
    x348 = x341*x93
    x349 = x341*x99
    x350 = x349*x56
    x351 = x343*x78
    x352 = x106 + x306*x344*x47 + x309*x350 - x344*x96 + x347*x57 - x348*x90 - x351*x97
    x353 = x343*x93
    x354 = 1.5*x348
    x355 = 18.0*r_cut*x10*x16*x21*x22*x3*x85 + 6.0*x10*x125*x16*x3*x85 - x133*x338 - x17*x182
    x356 = 1.5*x344
    x357 = x147*x341
    x358 = 6.0*e*r_cut*x341*x56*x91*x99 + 1.5*e*x125*x126*x341*x78 + 6.0*e*x343*x47*x56*x91*x99 + 1.5*r_cut*x3*x341*x55*x71*x78 - r_cut*x354 + 6.0*x111*x125*x341*x55*x71*x99 + x112 - x114 - x115 + x120 - x128*x351 - x136*x356 - x140*x350 - x143*x357 - x150*x347 + 1.5*x3*x343*x47*x55*x71*x78 - 1.5*x353*x47 + x355*x57
    x359 = 9.0*x344
    x360 = e*x174*x344
    x361 = 6.0*x341
    x362 = x351*x55
    x363 = x343*x99
    x364 = 24.0*x363
    x365 = x147*x343
    x366 = x344*x55
    x367 = x349*x55
    x368 = x172*x341
    x369 = x126*x368
    x370 = 3.0*x347
    x371 = x370*x78
    x372 = -x17*x223 + x17*x224 - x17*x225 - x226*x346 + x228*x229*x338 + 18.0*x338*x86
    x373 = -x121*x370 - x127*x371 - x129*x362 + x140*x351 - x155 - x157 + x159 - x163 + x167 - x168*x353 + x170*x359 - x173*x360 + x175*x204*x350 + x179*x361 + x182*x362 + x186*x188*x343 + x188*x361 - x189*x364 + x193*x202*x349 + x194*x364 - x196*x365 + x198*x356 - x199*x359 + x200*x366 - x203*x350 - x207*x367 - x210*x357 + x212*x368 - x215*x369 + x220*x369 - x231*x355 + x232*x371 + x233*x347 - x354 + x372*x57
    x374 = 180.0*x363
    x375 = x271*x341
    x376 = 15.0*x347
    x377 = 5.0*x355
    return [60*r_cut*x107*x36*x38*x61 + 5*r_cut*x110*x234*x36*x38 + 120*x1*x59 - x107*x109 + 20*x110*x152*x36*x38 - 20*x152*x153 - x234*x236 - x286*(22.5*e*r_cut*x12*x126*x34*x54*x78 + 60.0*e*r_cut*x12*x34*x56*x89*x91*x99 + 15.0*e*x12*x125*x126*x34*x78*x89 + 300.0*e*x12*x125*x174*x34*x54*x56*x99 + 45.0*e*x12*x126*x34*x47*x73*x78 + 20.0*e*x12*x134*x34*x47*x56*x91*x99 + 150.0*e*x12*x172*x174*x34*x56*x73*x99 + 52.5*e*x12*x252*x253*x34*x54*x78 + 30.0*e*x12*x34*x56*x73*x91*x99 + 90.0*r_cut*x111*x12*x34*x54*x55*x71*x99 + 15.0*r_cut*x12*x3*x34*x55*x71*x78*x89 + 720.0*x111*x12*x125*x144*x145*x34*x54*x85 + 60.0*x111*x12*x125*x34*x55*x71*x89*x99 + 270.0*x111*x12*x130*x252*x34*x54*x55*x99 + 360.0*x111*x12*x144*x145*x172*x34*x73*x85 + 180.0*x111*x12*x34*x47*x55*x71*x73*x99 + 960.0*x12*x125*x126*x217*x218*x34*x37*x54 + 60.0*x12*x125*x126*x34*x37*x54*x99 + 75.0*x12*x125*x130*x3*x34*x54*x55*x78 + 480.0*x12*x126*x172*x217*x218*x34*x37*x73 + 30.0*x12*x126*x172*x34*x37*x73*x99 + 37.5*x12*x130*x172*x3*x34*x55*x73*x78 + 5.0*x12*x134*x3*x34*x47*x55*x71*x78 + 720.0*x12*x144*x174*x252*x34*x37*x54*x56 + 960.0*x12*x145*x217*x237*x252*x34*x54*x85 + 7.5*x12*x3*x34*x55*x71*x73*x78 + x12*x34*x57*(300.0*x125*x130*x3 + 2100.0*x125*x19*x227*x3*x5 - x20*x282 - x20*x284 - x228*x265 - 90.0*x261) - x122*x279 - 15.0*x123*x151 - x124*x146*x274 - x124*x255*x277 - x124*x275*x276 - 15.0*x136*x79*x89 - 60.0*x141*x255 - x142*x263 - x142*x266 - x148*x268 - 60.0*x151*x189*x99 - x151*x280 - 7.5*x169 - x175*x250*x98 - x176*x251 - x178*x184*x260 - x197*x259 - 45.0*x199*x98 - x201*x264 - x208*x269 - x222*x279 - x230*x285*x35 + x239 - x240*x272*x278 + x241 - x242 - x245 - x248 + x249 - x256*x257 - x257*x258 - 22.5*x262*x80 - x270*x272) + 120*x36*x38*x61*x81 - x39*x81*x82 - x59*x60, x337, x337, 5*r_cut*x110*x36*x373*x38 + 60*r_cut*x352*x36*x38*x61 + 120*x1*x342 - x109*x352 + 20*x110*x358*x36*x38 - x236*x373 - x286*(22.5*e*r_cut*x126*x341*x78 + 60.0*e*r_cut*x347*x56*x91*x99 + 15.0*e*x125*x126*x347*x78 + 300.0*e*x125*x174*x341*x56*x99 + 45.0*e*x126*x343*x47*x78 + 150.0*e*x172*x174*x343*x56*x99 + 52.5*e*x252*x253*x341*x78 + 30.0*e*x343*x56*x91*x99 + 20.0*e*x355*x47*x56*x91*x99 + 90.0*r_cut*x111*x341*x55*x71*x99 + 15.0*r_cut*x3*x347*x55*x71*x78 + 720.0*x111*x125*x144*x145*x341*x85 + 60.0*x111*x125*x347*x55*x71*x99 + 270.0*x111*x130*x252*x341*x55*x99 + 360.0*x111*x144*x145*x172*x343*x85 + 180.0*x111*x343*x47*x55*x71*x99 - x121*x377 - x123*x376 + 960.0*x125*x126*x217*x218*x341*x37 + 60.0*x125*x126*x341*x37*x99 + 75.0*x125*x130*x3*x341*x55*x78 + 480.0*x126*x172*x217*x218*x343*x37 + 30.0*x126*x172*x343*x37*x99 - x127*x377*x78 + 37.5*x130*x172*x3*x343*x55*x78 - x136*x376*x78 + 720.0*x144*x174*x252*x341*x37*x56 + 960.0*x145*x217*x237*x252*x341*x85 - x146*x274*x341 - x175*x250*x351 - x178*x260*x343 - 60.0*x189*x347*x99 - 45.0*x199*x351 - x239 - x240*x278*x375 - x241 + x242 + x245 + x248 - x249 - x251*x360 - x255*x277*x341 - 60.0*x255*x349 - x256*x374 - x258*x374 - x259*x365 - 22.5*x262*x344 - x263*x350 - x264*x366 - x266*x350 - x268*x357 - x269*x367 - x270*x375 - x275*x276*x341 - x280*x347 - x285*x372 + 7.5*x3*x343*x55*x71*x78 + 5.0*x3*x355*x47*x55*x71*x78 - 7.5*x353 + x57*(90.0*r_cut*x10*x16*x3*x85 + 2100.0*x10*x125*x16*x21*x22*x227*x3 + 210.0*x10*x16*x227*x252*x3 - 300.0*x132*x17 - x282*x338 - x284*x338)) - x299*x345 - x321*x358 - x342*x60 + 120*x345*x36*x38*x61]


deriv_fns = [deriv_0, deriv_1, deriv_2, deriv_3, deriv_4, deriv_5]

hessian_fns = [hessian_0, hessian_1, hessian_2, hessian_3, hessian_4, hessian_5]
